...
--changeset SYSTEM:CLICK_STREAM_RESOLUTION runOnChange:true splitStatements:false OBJECT_TYPE:VIEW
use database EDM_VIEWS_PRD;
use schema DW_VIEWS;

CREATE OR REPLACE VIEW Click_Stream_Resolution
(
Resolution_Nm comment 'The resolution of the monitor. Used in the Monitor resolution dimension. Uses resolution.tsv lookup table.'
,Resolution_Id comment 'Numeric ID representing the resolution of the monitor. Used in the Monitor resolution dimension. Uses resolution.tsv lookup table'
...
COPY GRANTS comment = 'View For Click_Stream_Stream_Resolution' AS 
SELECT
Resolution_Nm
,Resolution_Id
...
FROM 
EDM_CONFIRMED_PRD.DW_C_USER_ACTIVITY.Click_Stream_Resolution;