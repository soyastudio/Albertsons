...
--changeset SYSTEM:CLICK_STREAM_PLUGIN runOnChange:true splitStatements:false OBJECT_TYPE:TABLE
use database EDM_CONFIRMED_PRD;
use schema DW_C_USER_ACTIVITY;

create or replace TABLE CLICK_STREAM_PLUGIN (
	PLUGIN_NM VARCHAR(250) NOT NULL COMMENT 'No longer used. List of numeric ID’s that correspond to plugins available within the browser. Uses plugins.tsv lookup',
	...
	PLUGIN_ID VARCHAR(50) COMMENT 'Numeric ID’s that correspond to plugins available within the browser. Uses plugins.tsv lookup.',
	...
	primary key (PLUGIN_NM, DW_FIRST_EFFECTIVE_DT, DW_LAST_EFFECTIVE_DT)
);