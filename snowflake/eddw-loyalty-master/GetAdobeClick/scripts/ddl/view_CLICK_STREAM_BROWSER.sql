...
--changeset SYSTEM:CLICK_STREAM_BROWSER runOnChange:true splitStatements:false OBJECT_TYPE:VIEW
use database EDM_VIEWS_PRD;
use schema DW_VIEWS;

CREATE OR REPLACE VIEW Click_Stream_Browser 
(
Browser_Nm comment 'Name/Version of the browser'
,Browser_Id Comment 'Numeric ID of the browser. References the browser.tsv lookup table.'
,Browser_Type_Nm comment 'Company developing browser (MICROSOFT for Internet Explorer)'      
...
COPY GRANTS comment = 'VIEW For Click_Stream_Browser' 
AS
SELECT
   Browser_Nm 
  ,Browser_Id  
...
FROM  EDM_CONFIRMED_PRD.DW_C_USER_ACTIVITY.Click_Stream_Browser;