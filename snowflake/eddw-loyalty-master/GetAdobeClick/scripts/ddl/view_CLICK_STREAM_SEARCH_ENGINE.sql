...
--changeset SYSTEM:CLICK_STREAM_SEARCH_ENGINE runOnChange:true splitStatements:false OBJECT_TYPE:VIEW
use database EDM_VIEWS_PRD;
use schema DW_VIEWS;

CREATE OR REPLACE VIEW Click_Stream_Search_Engine
(
  Search_Engine_Nm  comment 'The Search Engine that referred the visitor to your site. Uses search_engines.tsv lookup.'
  ,Search_Engine_Id comment 'Numeric ID representing the Search Engine that referred the visitor to your site. Uses search_engines.tsv lookup.'
...
COPY GRANTS
comment = 'VIEW For Click_Stream_Search_Engine ' 
AS
SELECT
   Search_Engine_Nm      
  ,Search_Engine_Id  
...
FROM EDM_CONFIRMED_PRD.DW_C_USER_ACTIVITY.Click_Stream_Search_Engine;