...
--changeset SYSTEM:CLICK_STREAM_LANGUAGE runOnChange:true splitStatements:false OBJECT_TYPE:VIEW
use database EDM_VIEWS_PRD;
use schema DW_VIEWS;

CREATE OR REPLACE VIEW Click_Stream_Language
(
  Language_Nm comment 'Numeric ID of language. Uses languages.tsv lookup table.'
  ,Language_Id comment 'Numeric ID of language. Uses languages.tsv lookup table.'
...
COPY GRANTS
comment = 'VIEW For Click_Stream_Language' 
AS
SELECT
   Language_Nm
  ,Language_Id  
...
FROM EDM_CONFIRMED_PRD.DW_C_USER_ACTIVITY.Click_Stream_Language;