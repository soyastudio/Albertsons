...
--changeset SYSTEM:SP_GetNPSSurvey_To_BIM_load runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database EDM_CONFIRMED_PRD;
use schema DW_APPL;

CREATE OR REPLACE PROCEDURE EDM_CONFIRMED_PRD.DW_APPL.SP_GETNPSSURVEY_TO_BIM_LOAD()
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$

   var sub_proc_nms = ['SP_GetNPSSURVEY_TO_BIM_LOAD_NET_PROMOTER_SCORE_SURVEY',
					   'SP_GetNPSSURVEY_TO_BIM_LOAD_SURVEY_RESPONSE'
					  ]
	...
	var src_tbl = `${ref_db}.${app_schema}.GetNPSSurvey_Flat_R_STREAM`;
	var src_rerun_tbl = `${ref_db}.${wrk_schema}.GetNPSSurvey_Flat_Rerun`;
    var src_wrk_tbl = `${ref_db}.${wrk_schema}.GetNPSSurvey_Flat_wrk`;
