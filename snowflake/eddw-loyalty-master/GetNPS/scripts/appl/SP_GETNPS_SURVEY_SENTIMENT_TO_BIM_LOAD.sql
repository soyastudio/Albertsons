...
--changeset SYSTEM:SP_GETNPS_SURVEY_SENTIMENT_TO_BIM_LOAD runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE <<EDM_DB_NAME>>.DW_APPL.SP_GETNPS_SURVEY_SENTIMENT_TO_BIM_LOAD()
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$

   var sub_proc_nms = ['SP_GetNPSdetails_TO_BIM_LOAD_SURVEY_SENTIMENT'
					  ]
	...
	var src_tbl = `${ref_db}.${app_schema}.GET_SURVEY_SENTIMENT_Flat_R_STREAM`;
	var src_rerun_tbl = `${cnf_db}.${wrk_schema}.GET_SURVEY_SENTIMENT_FLAT_RERUN`;
    var src_wrk_tbl = `${cnf_db}.${wrk_schema}.GET_SURVEY_SENTIMENT_Flat_wrk`;
	...
	var sql_ins_rerun_tbl = `CREATE OR REPLACE TRANSIENT TABLE ${src_rerun_tbl} as SELECT * FROM ${src_wrk_tbl}`;
