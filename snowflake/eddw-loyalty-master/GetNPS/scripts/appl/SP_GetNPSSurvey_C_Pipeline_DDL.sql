...
--changeset SYSTEM:SP_GetNPSSurvey_C_Pipeline_DDL runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database EDM_CONFIRMED_PRD;
use schema DW_APPL;

CREATE OR REPLACE PROCEDURE EDM_CONFIRMED_PRD.DW_APPL.SP_GETNPSSURVEY_C_PIPELINE_DDL()
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
   ...
   try {
    ...
    // Task for FLAT Table
	var create_task = `CREATE OR REPLACE TASK SP_GetNPSSurvey_TO_BIM_LOAD_TASK
	WAREHOUSE = '${warehouse}'
	SCHEDULE = '30 minute'
	WHEN 
    SYSTEM$STREAM_HAS_DATA('${ref_db}.${app_schema}.GETNPSSURVEY_FLAT_R_STREAM')
	AS 
	CALL SP_GetNPSSurvey_To_BIM_load();`
    ...
    // To resume the task
	var resume_task = `ALTER TASK SP_GetNPSSurvey_TO_BIM_LOAD_TASK resume;`
