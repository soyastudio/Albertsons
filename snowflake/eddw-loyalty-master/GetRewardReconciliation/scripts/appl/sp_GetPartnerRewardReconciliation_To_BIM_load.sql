...
--changeset SYSTEM:sp_GetPartnerRewardReconciliation_To_BIM_load runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE <<EDM_DB_NAME>>.DW_APPL.SP_GETPARTNERREWARDRECONCILIATION_TO_BIM_LOAD()
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$

   var sub_proc_nms = ['SP_GetPartnerRewardReconciliation_TO_BIM_LOAD_Business_Partner_Reward_Reconciliation','SP_GetPartnerRewardReconciliation_TO_BIM_LOAD_Business_Partner_Reward_Reconciliation_Error']
    ...
    var src_tbl = `${ref_db}.${app_schema}.GetPartnerRewardReconciliation_Flat_R_Stream`;
	var src_rerun_tbl = `${ref_db}.${wrk_schema}.GetPartnerRewardReconciliation_Flat_Rerun`;
    var src_wrk_tbl = `${ref_db}.${wrk_schema}.GetPartnerRewardReconciliation_Flat_wrk`;
