...
--changeset SYSTEM:SP_GetMeal_Plan_Restriction_TO_BIM_LOAD_Meal_Plan_Restriction runOnChange:true splitStatements:false OBJECT_TYPE:SP
Use database <<EDM_DB_NAME>>;
Use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE SP_GetMeal_Plan_Restriction_TO_BIM_LOAD_Meal_Plan_Restriction(SRC_WRK_TBL VARCHAR, CNF_DB VARCHAR, C_LOYAL VARCHAR, C_STAGE VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS 
$$
// ************** Load for Meal_Plan_Restriction table BEGIN *****************
		var src_wrk_tbl = SRC_WRK_TBL;
		var temp_wrk_tbl = `${CNF_DB}.${C_STAGE}.Meal_Plan_Restriction_Tmp_WRK`;
		var cnf_schema = C_LOYAL;
		var tgt_wrk_tbl = `${CNF_DB}.${C_STAGE}.Meal_Plan_Restriction_WRK`;
		var tgt_tbl = `${CNF_DB}.${cnf_schema}.Meal_Plan_Restriction`;
		var src_rerun_tbl = `${CNF_DB}.${C_STAGE}.Meal_Plan_Restriction_Flat_RERUN`;
                            ...
                            FROM ( SELECT
								...
								FROM ( SELECT 
											...
											FALSE AS DW_LOGICAL_DELETE_IND,
										  	Row_number() OVER (
											 PARTITION BY Meal_Plan_Restriction_Id
											 order by DW_CreateTs DESC) as rn 
											  FROM(
													...
													FROM
													  (
													  SELECT
													  id as Meal_Plan_Restriction_Id , 
													  name as Meal_Plan_Restriction_Nm  ,     
													  DW_CreateTs ,         
...
var sql_Delete_Check = `UPDATE ${tgt_tbl} as tgt
					...
					DW_LOGICAL_DELETE_IND = TRUE 
                     from ( 
					       SELECT 
						   Meal_Plan_Restriction_Id as ID 
						   from ${tgt_tbl} as a
						   left join ${temp_wrk_tbl} as b
						   on a.Meal_Plan_Restriction_Id = b.ID
						   and a.DW_CURRENT_VERSION_IND = TRUE 
						   and a.DW_LOGICAL_DELETE_IND = FALSE
						   where b.ID is null
						   )del
				      where tgt.Meal_Plan_Restriction_Id = del.ID
...
var sql_delete = `delete from ${tgt_tbl} 
                 where Meal_Plan_Restriction_Id in ( select Meal_Plan_Restriction_Id from ${tgt_wrk_tbl})`;		
// Processing Inserts
var sql_inserts = `INSERT INTO ${tgt_tbl} 
								    ...
								    and Meal_Plan_Restriction_Nm is Not Null
									`;
...
try {
	   ...
	   return `Loading of table ${tgt_tbl} Failed with error: ${err}` ;   // Return a error message.
                ...
                // *************Load for Meal_Plan_Restriction Table ENDS *****************
