...
--changeset SYSTEM:SP_GetRetail_Customer_MealPlan_Profile_TO_BIM_LOAD_Retail_Customer_MealPlan_Profile runOnChange:true splitStatements:false OBJECT_TYPE:SP
Use database <<EDM_DB_NAME>>;
Use schema <<EDM_DB_NAME>>.DW_APPL;
                                    
CREATE OR REPLACE PROCEDURE SP_GetRetail_Customer_MealPlan_Profile_TO_BIM_LOAD_Retail_Customer_MealPlan_Profile(SRC_WRK_TBL VARCHAR, CNF_DB VARCHAR, C_LOYAL VARCHAR, C_STAGE VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS 
$$
// ************** Load for Retail_Customer_MealPlan_Profile table BEGIN *****************
		var src_wrk_tbl = SRC_WRK_TBL;
		var temp_wrk_tbl = `${CNF_DB}.${C_STAGE}.Retail_Customer_MealPlan_Profile_Tmp_WRK`;
		var cnf_schema = C_LOYAL;
		var tgt_wrk_tbl = `${CNF_DB}.${C_STAGE}.Retail_Customer_MealPlan_Profile_WRK`;
		var tgt_tbl = `${CNF_DB}.${cnf_schema}.Retail_Customer_MealPlan_Profile`;
		var src_rerun_tbl = `${CNF_DB}.${C_STAGE}.Retail_Customer_MealPlan_Profile_Flat_RERUN`;
                            ...
                            FROM ( SELECT
								...
								FROM ( SELECT 
										...
										FALSE AS DW_LOGICAL_DELETE_IND,
										  	Row_number() OVER (
											 PARTITION BY Retail_Customer_UUID,Meal_Plan_Profile_Id
											  ...
											  FROM(
													...
													FROM
													  ...
													  SELECT
														...
														id as Meal_Plan_Profile_Id ,
														profilename as Meal_Plan_Profile_Nm ,
														servingCount as Recipe_Serving_Cnt ,
														recipeTypename as Meal_Plan_Diet_Type_Nm ,
														recipeTypeid as Meal_Plan_Diet_Type_Cd ,
														DW_CreateTs ,
...
var sql_Delete_Check = `UPDATE ${tgt_tbl} as tgt
					...
					DW_LOGICAL_DELETE_IND = TRUE 
                     from ( 
					  ...
					  AND tgt.Meal_Plan_Profile_Id = del.ID
...
var sql_delete = `delete from ${tgt_tbl} 
                 where (Retail_Customer_UUID,Meal_Plan_Profile_Id) in ( select Retail_Customer_UUID,Meal_Plan_Profile_Id from ${tgt_wrk_tbl})`;					  
...
try {
	   ...
	   return `Loading of table ${tgt_tbl} Failed with error: ${err}` ;   // Return a error message.
                ...
                // ************** Load for Retail_Customer_MealPlan_Profile Table ENDS *****************
