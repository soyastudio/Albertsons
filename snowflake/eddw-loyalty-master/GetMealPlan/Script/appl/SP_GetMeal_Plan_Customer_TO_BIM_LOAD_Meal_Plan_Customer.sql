...
--changeset SYSTEM:SP_GetMeal_Plan_Customer_TO_BIM_LOAD_Meal_Plan_Customer runOnChange:true splitStatements:false OBJECT_TYPE:SP
Use database <<EDM_DB_NAME>>;
Use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE SP_GetMeal_Plan_Customer_TO_BIM_LOAD_Meal_Plan_Customer(SRC_WRK_TBL VARCHAR, CNF_DB VARCHAR, C_LOYAL VARCHAR, C_STAGE VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS 
$$
// ************** Load for Retail_Customer_Favorite table BEGIN *****************
		...
		var temp_wrk_tbl = `${CNF_DB}.${C_STAGE}.Meal_Plan_Customer_tmp_WRK`;
		var cnf_schema = C_LOYAL;
		var tgt_wrk_tbl = `${CNF_DB}.${C_STAGE}.Meal_Plan_Customer_WRK`;
		var tgt_tbl = `${CNF_DB}.${cnf_schema}.Meal_Plan_Customer`;
		var src_rerun_tbl = `${CNF_DB}.${C_STAGE}.Meal_Plan_Customer_Flat_RERUN`;
...
// *********************** Load for Meal_Plan_Customer table BEGIN *****************
								...
								FROM ( 
                                      SELECT  
											...
											FALSE AS DW_LOGICAL_DELETE_IND,
										  	Row_number() OVER (
											 PARTITION BY Retail_Customer_UUID
											 order by (DW_CreateTs) DESC) as rn 
											  FROM(
													...
													FROM
													  (
													  SELECT
													  id as Retail_Customer_UUID ,
													  role as Meal_Plan_Role_Nm,
													  to_timestamp_ntz(createddate) as Source_Create_Ts ,         
													  to_timestamp_ntz(updateddate) as Source_Last_Update_Ts,
													  DW_CreateTs ,         
...
var sql_Delete_Check = `UPDATE ${tgt_tbl} as tgt
					...
					DW_LOGICAL_DELETE_IND = TRUE 
                     from ( 
					       SELECT 
						   Retail_Customer_UUID as ID 
						   from ${tgt_tbl} as a
						   left join ${temp_wrk_tbl} as b
						   on a.Retail_Customer_UUID = b.ID
						   and a.DW_CURRENT_VERSION_IND = TRUE 
						   and a.DW_LOGICAL_DELETE_IND = FALSE
						   where b.ID is null
						   )del
				      where tgt.Retail_Customer_UUID = del.ID
...
var sql_delete = `delete from ${tgt_tbl} 
                 where Retail_Customer_UUID in ( select Retail_Customer_UUID from ${tgt_wrk_tbl})`;  					  
...
var sql_inserts = `INSERT INTO ${tgt_tbl} 
								   (
									...
									AND Meal_Plan_Role_Nm is NOT NULL
									AND Retail_Customer_UUID !=''
									`;
...
try {
	   ...
	   return `Loading of table ${tgt_tbl} Failed with error: ${err}` ;   // Return a error message.
                ...
                // ************** Load for Meal_Plan_Customer Table ENDs *****************
