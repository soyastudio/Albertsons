...
--changeset SYSTEM:SP_GetRetail_Customer_Favorite_TO_BIM_LOAD_Retail_Customer_Favorite runOnChange:true splitStatements:false OBJECT_TYPE:SP
Use database <<EDM_DB_NAME>>;
Use schema <<EDM_DB_NAME>>.DW_APPL;


CREATE OR REPLACE PROCEDURE SP_GetRetail_Customer_Favorite_TO_BIM_LOAD_Retail_Customer_Favorite(SRC_WRK_TBL VARCHAR, CNF_DB VARCHAR, C_LOYAL VARCHAR, C_STAGE VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS 
$$
// ************** Load for Retail_Customer_Favorite table BEGIN *****************
		var src_wrk_tbl = SRC_WRK_TBL;
		var temp_wrk_tbl = `${CNF_DB}.${C_STAGE}.Retail_Customer_Favorite_tmp_WRK`;
		var cnf_schema = C_LOYAL;
		var tgt_wrk_tbl = `${CNF_DB}.${C_STAGE}.Retail_Customer_Favorite_WRK`;
		var tgt_tbl = `${CNF_DB}.${cnf_schema}.Retail_Customer_Favorite`;
		var src_rerun_tbl = `${CNF_DB}.${C_STAGE}.Retail_Customer_Favorite_Flat_RERUN`;
								...
								FROM ( 
                                      SELECT  
											...
											FALSE AS DW_LOGICAL_DELETE_IND,
										  	Row_number() OVER (
											 PARTITION BY Retail_Customer_UUID,Recipe_Favorite_Id
											 order by (DW_CreateTs) DESC) as rn 
											  FROM(
													...
													FROM
													  ...
													  favorite_id as Recipe_Favorite_Id,
													  recipeId as Recipe_Variant_Id ,
													  to_timestamp_ntz(createddate) as Source_Create_Ts ,         
													  to_timestamp_ntz(updateddate) as Source_Update_Ts,
													  DW_CreateTs ,         
...
var sql_delete = `delete from ${tgt_tbl} 
                 where (Retail_Customer_UUID,Recipe_Favorite_Id) in ( select Retail_Customer_UUID,Recipe_Favorite_Id from ${tgt_wrk_tbl})`;
...
var sql_inserts = `INSERT INTO ${tgt_tbl} 
								   (
								    ...
								    and Recipe_Favorite_Id is Not Null
									`;
...
try {
	   ...
	   return `Loading of table ${tgt_tbl} Failed with error: ${err}` ;   // Return a error message.
                ...
                // ************** Load for Retail_Customer_Favorite Table ENDs *****************
