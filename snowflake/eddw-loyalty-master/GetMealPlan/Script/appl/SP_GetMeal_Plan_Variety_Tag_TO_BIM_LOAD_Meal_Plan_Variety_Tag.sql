...
--changeset SYSTEM:SP_GetMeal_Plan_Variety_Tag_TO_BIM_LOAD_Meal_Plan_Variety_Tag runOnChange:true splitStatements:false OBJECT_TYPE:SP
Use database <<EDM_DB_NAME>>;
Use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE SP_GetMeal_Plan_Variety_Tag_TO_BIM_LOAD_Meal_Plan_Variety_Tag(SRC_WRK_TBL VARCHAR, CNF_DB VARCHAR, C_LOYAL VARCHAR, C_STAGE VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS 
$$
// ************** Load for Meal_Plan_Variety_Tag table BEGIN *****************
		var src_wrk_tbl = SRC_WRK_TBL;
		var temp_wrk_tbl = `${CNF_DB}.${C_STAGE}.Meal_Plan_Variety_Tag_Tmp_WRK`;
		var cnf_schema = C_LOYAL;
		var tgt_wrk_tbl = `${CNF_DB}.${C_STAGE}.Meal_Plan_Variety_Tag_WRK`;
		var tgt_tbl = `${CNF_DB}.${cnf_schema}.Meal_Plan_Variety_Tag`;
		var src_rerun_tbl = `${CNF_DB}.${C_STAGE}.Meal_Plan_Variety_Tag_Flat_RERUN`;
                            ...
                            FROM ( SELECT
								...
								FROM ( SELECT 
											...
											FALSE AS DW_LOGICAL_DELETE_IND,
										  	Row_number() OVER (
											 PARTITION BY Recipe_Variety_Id
											 order by DW_CreateTs DESC) as rn 
											  FROM(
													...
													FROM
													  (
													  SELECT
													  id as Recipe_Variety_Id , 
													  name as Recipe_Variety_Type_Nm  ,     
													  DW_CreateTs ,         
...
var sql_delete = `delete from ${tgt_tbl} 
                 where Recipe_Variety_Id in ( select Recipe_Variety_Id from ${tgt_wrk_tbl})`;					  
...
var sql_inserts = `INSERT INTO ${tgt_tbl} 
									...
									FROM ${tgt_wrk_tbl}
									WHERE
									Recipe_Variety_Id is Not Null
								    and Recipe_Variety_Type_Nm is Not Null
									`;
...
try {
	   ...
	   return `Loading of table ${tgt_tbl} Failed with error: ${err}` ;   // Return a error message.
                ...
                // **************        Load for Meal_Plan_Variety_Tag Table ENDS *****************
