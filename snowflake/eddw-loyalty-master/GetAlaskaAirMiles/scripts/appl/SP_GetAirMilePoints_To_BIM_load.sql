...
--changeset SYSTEM:SP_GetAirMilePoints_To_BIM_load runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE <<EDM_DB_NAME>>.DW_APPL.SP_GETAIRMILEPOINTS_TO_BIM_LOAD()
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$

   var sub_proc_nms = ['SP_GETAIRMILEPOINTS_To_BIM_load_Air_Mile_Points_DETAIL'
					  ,'SP_GetAirMilePoints_To_BIM_load_Air_Mile_Points_Detail_Attachment'
					  ,'SP_GETAIRMILEPOINTS_To_BIM_load_Air_Mile_Points_Summary'
					  ,'SP_GetAirMilePoints_To_BIM_load_Air_Mile_Points_Summary_Attachment']
    ...
    var src_tbl = `${ref_db}.${app_schema}.GetAirMilePoints_Flat_R_Stream`;
	var src_rerun_tbl = `${ref_db}.${wrk_schema}.GetAirMilePoints_Flat_Rerun`;
    var src_wrk_tbl = `${ref_db}.${wrk_schema}.GetAirMilePoints_Flat_wrk`;
