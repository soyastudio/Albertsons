...
--changeset SYSTEM:SP_F_ORDER_TRANSACTION_LOAD_gwt runOnChange:true splitStatements:false OBJECT_TYPE:SP
...
SET SCHEDULE = 'USING CRON 30 14 * * * Asia/Kolkata'
 ;
...
final_rec AS (
    SELECT gwt.txn_dte as transaction_dt,
        ...
        INNER JOIN ${db_views}.DW_VIEWS.D0_FISCAL_DAY cal ON gwt.txn_dte=cal.calendar_dt        
	        ...
	        WHERE GWT.TXN_TM <= TO_TIMESTAMP('${current_watermark}') AND
