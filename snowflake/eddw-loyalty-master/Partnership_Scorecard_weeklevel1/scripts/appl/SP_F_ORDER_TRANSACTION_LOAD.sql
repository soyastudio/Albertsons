...
SET SCHEDULE = 'USING CRON 0 18 * * * Asia/Kolkata'
 ;
...
ret_wm_ts2.next();
var last_watermark_from_tble1 = ret_wm_ts2.getColumnValue(1);

var current_wm_id1=
snowflake.createStatement({sqlText: `SELECT TO_CHAR(MAX(transaction_dt)) FROM ${db_views}.dw_views.F_order_transaction txnc 
                              WHERE transaction_dt  >= CURRENT_DATE - 365;`})
var current_wm_id2=current_wm_id1.execute();
current_wm_id2.next();
var last_watermark_from_tble2 = current_wm_id2.getColumnValue(1);
...
results_array[2]=last_watermark2;
...
final_rec AS (
    SELECT gwt.txn_dte as transaction_dt,
		...
		SUM(gwt.total_item_qty) as item_qty,
 		case when /*thc.register_nbr = 99  and dug.txn_id is not null and */dug.delivery_type_dsc like '%DUG%' then 
		TRUE 
		else 
		FALSE 
        ...
        LEFT JOIN ${db_views}.DW_VIEWS.D0_FISCAL_DAY cal ON gwt.txn_dte=cal.calendar_dt        
	        ...
	        WHERE  
						GWT.TXN_TM <= to_timestamp('${current_watermark}') AND 
                          GWT.TXN_TM >= TO_TIMESTAMP('${last_watermark2}')						
