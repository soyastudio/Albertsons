...
--changeset SYSTEM:SP_GETUBER_TO_BIM_LOAD_Partner_Grocery_Order_Customer runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE SP_GETUBER_TO_BIM_LOAD_PARTNER_GROCERY_ORDER_CUSTOMER(SRC_WRK_TBL1 VARCHAR, 
SRC_WRK_TBL2 VARCHAR, CNF_DB VARCHAR, C_LOYAL VARCHAR, C_STAGE VARCHAR, C_CUST VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
...
var src_wrk_tbl2 = SRC_WRK_TBL2;
var cnf_schema = C_LOYAL;
var lkp_schema = C_CUST;
var wrk_schema = C_STAGE;
var tgt_wrk_tbl = `${CNF_DB}.${wrk_schema}.Partner_Grocery_Order_Customer_WRK`;
var tgt_tbl = `${CNF_DB}.${cnf_schema}.Partner_Grocery_Order_Customer`;
var lkp_tbl1 = `${CNF_DB}.${lkp_schema}.RETAIL_CUSTOMER`;
...
(
SELECT DISTINCT
lower(a.Source_Customer_Id) as Source_Customer_Id,
a.Loyalty_Phone_Nbr,
a.First_Nm,
a.Last_Nm,
a.Email_Address_Txt,
...
a.filename,
a.DW_Logical_delete_ind,
--row_number() over (PARTITION BY lower(a.Source_Customer_Id),a.Loyalty_Phone_Nbr,a.First_Nm,a.Last_Nm ORDER BY b.STORE_TXN_TS desc )as rn
row_number() over (PARTITION BY lower(a.Source_Customer_Id) ORDER BY b.STORE_TXN_TS,a.Loyalty_Phone_Nbr,a.First_Nm,a.Last_Nm desc )as rn
...
LAST_NM as Last_Nm,
EMAIL_ADDR_TXT as Email_Address_Txt,
FULL_PHONE_NBR as Contact_Phone_Nbr,
filename
FROM ${src_wrk_tbl1}
...
FROM ${src_wrk_tbl2}
//where lower(USER_ID) is not null
)flat2 on nvl(lower(flat1.Source_Customer_Id),'-1') = nvl(lower(flat2.USER_ID),'-1')
)
where lower(Source_Customer_Id) IS NOT NULL
OR LOYALTY_PHONE_NBR IS NOT NULL
UNION ALL
SELECT DISTINCT
lower(Source_Customer_Id) as Source_Customer_Id,
Loyalty_Phone_Nbr,
...
Contact_Phone_Nbr,
filename
FROM ${tgt_exp_tbl} where upper(filename) like '%UBER%'
) src
LEFT JOIN
( SELECT Retail_Customer_UUID,First_Nm, Last_Nm
FROM ${lkp_tbl1}
WHERE DW_CURRENT_VERSION_IND = TRUE
AND DW_LOGICAL_DELETE_IND = FALSE
) LKP
ON src.First_Nm = LKP.First_Nm
AND src.Last_Nm = LKP.Last_Nm
) a
 left join ${src_wrk_tbl2} b on lower(a.Source_Customer_Id)= lower(b.USER_ID)
 
)
SELECT DISTINCT
lower(Source_Customer_Id),
Loyalty_Phone_Nbr,
First_Nm,
Last_Nm,
Email_Address_Txt,
...
Retail_Customer_UUID,
filename,
CASE WHEN Retail_Customer_UUID IS NULL THEN 'Retail_Customer_UUID is NULL'
END AS Exception_Reason,
current_timestamp AS dw_create_ts
FROM `+ tgt_wrk_tbl +`
WHERE Retail_Customer_UUID is NULL 
