...
--changeset SYSTEM:SP_GETFRESHPASS_SNOWPIPE_TO_LOAD_VARIANT runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME_R>>;
use schema <<EDM_DB_NAME_R>>.DW_APPL;

CREATE OR REPLACE PROCEDURE <<EDM_DB_NAME_R>>.DW_APPL.SP_GETFRESHPASS_SNOWPIPE_TO_LOAD_VARIANT()
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
	...
	var stream_nm = `${ref_db}.${app_schema}.${variant_nm}_R_STREAM`;
	...
	var flat_src_tbl = `${ref_db}.${app_schema}.GetFreshpass_Flat_R_STREAM`;
	var flat_src_rerun_tbl = `${cnf_db}.${cnf_wrk_schema}.GetFreshpass_Flat_Rerun`;	
		
	// If the topic folder  is  its-${env}-kafka-topics
	
	var topic_nm_location = 'OCSP_02_SubsriptionStatusChange/';
	...
	try {
        ...
        throw `Creation of stream on variant table failed with error  ${stream_nm} on table  ${ref_db}.${ref_schema}.${variant_nm} : ${err}`;   // Return a error message.
    }

   
	// check if rerun queue table exists otherwise create it
	var variant_sql_crt_rerun_tbl = `CREATE OR REPLACE TRANSIENT TABLE ${variant_src_rerun_tbl} DATA_RETENTION_TIME_IN_DAYS = 0 
	...
	var flat_sql_crt_rerun_tbl = `CREATE OR REPLACE TRANSIENT TABLE ${flat_src_rerun_tbl} DATA_RETENTION_TIME_IN_DAYS = 0 AS SELECT * FROM ${flat_src_tbl} where 1=2 `;
	try {
	...
	// Task for FLAT Table
	var create_task = `CREATE OR REPLACE TASK ESED_Freshpass_R_TASK
	WAREHOUSE = '${warehouse}'
	SCHEDULE = '1 minutes'
	WHEN
	SYSTEM$STREAM_HAS_DATA('${stream_nm}')
	AS 
	call sp_GetFreshpass_To_FLAT_load();`
	...
	/* To resume the task
	  var resume_task = `ALTER TASK ESED_Freshpass_R_TASK resume;`
