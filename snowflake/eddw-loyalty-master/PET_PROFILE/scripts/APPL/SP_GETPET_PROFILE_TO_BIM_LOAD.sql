...
--changeset SYSTEM:SP_GETPET_PROFILE_TO_BIM_LOAD runOnChange:true splitStatements:false OBJECT_TYPE:SP

use database <<EDM_DB_NAME>>;
use schema DW_APPL;

CREATE OR REPLACE PROCEDURE SP_GETPET_PROFILE_TO_BIM_LOAD()
RETURNS VARCHAR(16777216)
LANGUAGE JAVASCRIPT
AS 
$$
   
   var sub_proc_nms = ['SP_GetPet_Profile_Flat_To_BIM_Pet_Profile'
                       ,'SP_GetPet_Profile_Flat_To_BIM_Pet_Allergy'
					   ,'SP_GetPet_Profile_Flat_To_BIM_Pet_Breed'
					   ,'SP_GetPet_Profile_Flat_To_BIM_Pet_Medical_Condition'
					   ,'SP_GetPet_Profile_Flat_To_BIM_Pet_Medication'
					  ]
    ...
    try {
        ...
        var ref_schema = metaparams['C_LOYAL'];
        var cnf_schema = metaparams['C_LOYALTY'];
        var cnf_db = metaparams['CNF_DB'];
        var wrk_schema = metaparams['C_STAGE'];
        var ref_wrk_schema = metaparams['R_STAGE'];
    ...
    var src_tbl = `${ref_db}.${app_schema}.GetPet_Profile_Flat_R_Stream`;
	var src_rerun_tbl = `${ref_db}.${ref_wrk_schema}.GetPet_Profile_Flat_Rerun`;
    var src_wrk_tbl = `${ref_db}.${ref_wrk_schema}.GetPet_Profile_Flat_wrk`;
