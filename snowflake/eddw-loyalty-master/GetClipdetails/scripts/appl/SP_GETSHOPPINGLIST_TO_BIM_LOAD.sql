...
--changeset SYSTEM:SP_GETSHOPPINGLIST_TO_BIM_LOAD runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database EDM_CONFIRMED_PRD;
use schema DW_APPL;

CREATE OR REPLACE PROCEDURE EDM_CONFIRMED_PRD.DW_APPL.SP_GETSHOPPINGLIST_TO_BIM_LOAD()
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$

   var sub_proc_nms = ['SP_GetShoppingList_TO_BIM_LOAD_CLIP_HEADER',
					   'SP_GetShoppingList_TO_BIM_LOAD_CLIP_DETAILS'
					  ]
	...
	var src_tbl = `${ref_db}.${app_schema}.GetShoppingList_Flat_R_STREAM`;
	var src_rerun_tbl = `${ref_db}.${wrk_schema}.GetShoppingList_Flat_Rerun`;
    var src_wrk_tbl = `${ref_db}.${wrk_schema}.GetShoppingList_Flat_wrk`;
    var tmp_wrk_tbl = `${ref_db}.${wrk_schema}.GETSHOPPINGLIST_STREAM_LOAD`;
    ...
    var sql_crt_tmp_wrk_tbl = `insert into ${tmp_wrk_tbl}
    select * from ${src_wrk_tbl}`;
    try {
        snowflake.execute({ sqlText: sql_crt_tmp_wrk_tbl });
    } catch (err)  {
        throw `Creation of Source Work table ${sql_crt_tmp_wrk_tbl} Failed with error: ${err}`;   // Return a error message.
    }
