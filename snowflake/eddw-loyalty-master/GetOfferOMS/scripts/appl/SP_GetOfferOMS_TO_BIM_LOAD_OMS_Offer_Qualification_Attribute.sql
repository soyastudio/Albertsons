...
--changeset SYSTEM:SP_GetOfferOMS_TO_BIM_LOAD_OMS_Offer_Qualification_Attribute runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE SP_GETOFFEROMS_TO_BIM_LOAD_OMS_OFFER_QUALIFICATION_ATTRIBUTE(SRC_WRK_TBL VARCHAR(16777216), CNF_DB VARCHAR(16777216), C_PRODUCT VARCHAR(16777216), C_STAGE VARCHAR(16777216))
...
$$
	...
	var tgt_wrk_tbl = `${CNF_DB}.${C_STAGE}.OMS_Offer_Qualification_Attribute_WRK`;
	var tgt_tbl = `${CNF_DB}.${cnf_schema}.OMS_Offer_Qualification_Attribute`;
	var src_wrk_tmp_tbl = `${CNF_DB}.${C_STAGE}.OMS_Offer_Qualification_Attribute_src_wrk`;

                       
    // **************        Load for OMS_Offer_Qualification_Attribute table BEGIN *****************
    // identify  if the columns have changed between Target table and changed dataset and create a work table specific to the BIM table. 
	
	// Empty the source work table
		var sql_empty_src_wrk_tbl = `TRUNCATE TABLE `+ src_wrk_tmp_tbl +` `;
		try {
                            ...
                            ,row_number() over ( PARTITION BY OMS_Offer_Id
                             ...
                             WHERE OMS_Offer_Id IS NOT NULL AND PAYLOAD_QUALIFICATIONATTRIBUTES  IS NOT NULL	
								...
								FROM (
										...
										PAYLOAD_QUALIFICATIONATTRIBUTES.value:attributeCombo::string AS Attribute_Combo_Txt,
										filename, 
										rn
										FROM flat_tmp
								...
								Qualification_Attribute_Type_Txt is not NULL
								and rn = 1 
   `;
    try {
        snowflake.execute (
        ...
        return "Creation of OMS_Offer_Qualification_Attribute src_wrk_tmp_tbl table "+ src_wrk_tmp_tbl +" Failed with error: " + err;   // Return a error message.
        }

	var Insert_tgt_wrk_table = `INSERT INTO ${tgt_wrk_tbl} 
	SELECT DISTINCT
		src.OMS_Offer_Id,
		src.Qualification_Attribute_Type_Txt,
		src.Attribute_Combo_Txt,
		src.filename,
		src.DW_Logical_delete_ind,
		CASE 
			WHEN 
				tgt.OMS_Offer_Id is NULL AND
			tgt.Qualification_Attribute_Type_Txt is NULL
			THEN 'I' 
			ELSE 'U' 
		END as DML_Type,
		CASE 
	...
	src.Qualification_Attribute_Type_Txt = tgt.Qualification_Attribute_Type_Txt
	WHERE
		tgt.OMS_Offer_Id is NULL AND
		tgt.Qualification_Attribute_Type_Txt is NULL 
		 OR (
		 NVL(tgt.Attribute_Combo_Txt,'-1') <> NVL(src.Attribute_Combo_Txt,'-1') 
	...
	try {
		...
		,tgt.Qualification_Attribute_Type_Txt
		,tgt.Attribute_Combo_Txt
        ,src_wrk.FileName
        ...
        ,CASE WHEN DW_First_Effective_dt = CURRENT_DATE  THEN 1 Else 0 END as Sameday_chg_ind
            ...
            FROM ${src_wrk_tmp_tbl}
              ...
              ON src.Qualification_Attribute_Type_Txt = tgt.Qualification_Attribute_Type_Txt
           ...
           FROM ${src_wrk_tmp_tbl}
            ...
            AND dw_logical_delete_ind = FALSE
              ...
              and src.Qualification_Attribute_Type_Txt is NULL
            `;
try {
        snowflake.execute (
            {sqlText: sql_deletes  }
	...
	FROM ( 
		...
		WHERE 
			...
			tgt.Qualification_Attribute_Type_Txt = src.Qualification_Attribute_Type_Txt AND
	...
	SET 
		Attribute_Combo_Txt = src.Attribute_Combo_Txt,
	...
	try {
       ...
       return `Loading of table ${tgt_tbl} Failed with error: ${err}` ;   // Return a error message.
                ...
                // **************        Load for OMS_Offer_Qualification_Attribute table ENDs *****************
