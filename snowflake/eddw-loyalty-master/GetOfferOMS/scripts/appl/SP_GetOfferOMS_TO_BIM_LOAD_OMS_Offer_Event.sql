...
--changeset SYSTEM:SP_GetOfferOMS_TO_BIM_LOAD_OMS_Offer_Event runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE SP_GETOFFEROMS_TO_BIM_LOAD_OMS_OFFER_EVENT
...
$$
	...
	var tgt_wrk_tbl = `${CNF_DB}.${C_STAGE}.OMS_Offer_Event_WRK`;
	var tgt_tbl = `${CNF_DB}.${cnf_schema}.OMS_Offer_Event`;
	var src_wrk_tmp_tbl = `${CNF_DB}.${C_STAGE}.OMS_Offer_Event_src_wrk`;

                       
    // **************        Load for OMS_Offer_Event table BEGIN *****************
	    ...
	    var cr_src_wrk_tbl = `	INSERT INTO `+ src_wrk_tmp_tbl +`
                          with flat_tmp as
                              (		
										...
										FROM ` + src_wrk_tbl +`
										WHERE OMS_Offer_Id IS NOT NULL AND payload_events IS NOT NULL
								    ...
								    OMS_Offer_Event_Cd,
                                            ...
                                            FROM (
                                                      SELECT
						                                   OMS_Offer_Id,
							                           RegionIdmap.Key::string as OMS_Offer_Event_Cd,
                                                                                   RegionIdmap.Value::string as OMS_Offer_Event_Description_Txt,
                                                                                   filename,
									           rn
										   FROM flat_tmp
										   ,LATERAL FLATTEN(input => payload_events, outer => TRUE) as RegionIdMap													   
							                		 ) 				
                                                  WHERE rn = 1 AND
											      OMS_Offer_Id is not NULL AND
							                	  OMS_Offer_Event_Cd is not NULL`;
    try {
        snowflake.execute (
	...
	SELECT DISTINCT
		...
		CASE 
			...
			tgt.OMS_Offer_Event_Cd is NULL
			THEN 'I' 
			ELSE 'U' 
		END as DML_Type,
		CASE 
	...
	src.OMS_Offer_Event_Cd = tgt.OMS_Offer_Event_Cd
	WHERE
		tgt.OMS_Offer_Id is NULL AND
		tgt.OMS_Offer_Event_Cd is NULL 
		  OR (
	...
	try {
        ...
        ,CASE WHEN DW_First_Effective_dt = CURRENT_DATE  THEN 1 Else 0 END as Sameday_chg_ind
            ...
            FROM ${src_wrk_tmp_tbl}
              ...
              ON src.OMS_Offer_Event_Cd = tgt.OMS_Offer_Event_Cd
           ...
           FROM ${src_wrk_tmp_tbl}
            ...
            AND dw_logical_delete_ind = FALSE
              ...
              and src.OMS_Offer_Event_Cd is NULL
            `;
try {
        snowflake.execute (
            {sqlText: sql_deletes  }
	...
	FROM ( 
		...
		WHERE 
			...
			tgt.OMS_Offer_Event_Cd = src.OMS_Offer_Event_Cd AND
	...
	try {
       ...
       return `Loading of table ${tgt_tbl} Failed with error: ${err}` ;   // Return a error message.
                ...
                // **************        Load for OMS_Offer_Event table ENDs *****************
