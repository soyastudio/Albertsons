...
--changeset SYSTEM:SP_GetOfferOMS_TO_BIM_LOAD_OMS_Offer_Region runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE SP_GETOFFEROMS_TO_BIM_LOAD_OMS_OFFER_REGION(SRC_WRK_TBL VARCHAR, CNF_DB VARCHAR, C_PRODUCT VARCHAR, C_STAGE VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
	...
	var tgt_wrk_tbl = `${CNF_DB}.${C_STAGE}.OMS_Offer_Region_WRK`;
	var tgt_tbl = `${CNF_DB}.${cnf_schema}.OMS_Offer_Region`;
	var src_wrk_tmp_tbl = `${CNF_DB}.${C_STAGE}.OMS_Offer_Region_src_wrk`;

                       
    // **************        Load for OMS_Offer_Region table BEGIN *****************
	    ...
	    var cr_src_wrk_tbl = `	INSERT INTO `+ src_wrk_tmp_tbl +` 
                          with flat_tmp as
                              (		
										...
										FROM ` + src_wrk_tbl +`
										WHERE OMS_Offer_Id IS NOT NULL AND PAYLOAD_REGIONIDMAP IS NOT NULL
                                            ...
                                            FROM (
                                                      SELECT
						                                   OMS_Offer_Id,
							                               RegionIdmap.Key::string as OMS_Offer_Region_Cd,
                                                           RegionIdmap.Value::string as OMS_Offer_Region_Nm,			 
							                               filename,
														   rn
													 FROM flat_tmp
													 ,LATERAL FLATTEN(input => PAYLOAD_REGIONIDMAP, outer => TRUE) as RegionIdMap													   
							                		 ) 				
                                                  WHERE rn = 1 AND
											      OMS_Offer_Id is not NULL AND
							                	  OMS_Offer_Region_Cd is not NULL`;
    try {
	...
	SELECT DISTINCT
		src.OMS_Offer_Id,
		src.OMS_Offer_Region_Cd,
		src.OMS_Offer_Region_Nm,
		src.filename,
		src.DW_Logical_delete_ind,
		CASE 
			WHEN 
			tgt.OMS_Offer_Id is NULL AND
			tgt.OMS_Offer_Region_Cd is NULL
			THEN 'I' 
			ELSE 'U' 
		END as DML_Type,
		CASE 
	...
	src.OMS_Offer_Region_Cd = tgt.OMS_Offer_Region_Cd
	WHERE
		tgt.OMS_Offer_Id is NULL AND
		tgt.OMS_Offer_Region_Cd is NULL 
		  OR (
		 NVL(src.OMS_Offer_Region_Nm,'-1') <> NVL(tgt.OMS_Offer_Region_Nm,'-1') 
    ...
    snowflake.execute ({ sqlText: sql_truncate_wrk_tbl});
		...
		,tgt.OMS_Offer_Region_Cd
		,tgt.OMS_Offer_Region_Nm
        ,src_wrk.FileName
        ...
        ,CASE WHEN DW_First_Effective_dt = CURRENT_DATE  THEN 1 Else 0 END as Sameday_chg_ind
            ...
            FROM ${src_wrk_tmp_tbl}
              ...
              ON src.OMS_Offer_Region_Cd = tgt.OMS_Offer_Region_Cd
           ...
           FROM ${src_wrk_tmp_tbl}
            ...
            AND dw_logical_delete_ind = FALSE
              ...
              and src.OMS_Offer_Region_Cd is NULL
            `;
try {
        snowflake.execute (
            {sqlText: sql_deletes  }
	...
	FROM ( 
		...
		WHERE 
			...
			tgt.OMS_Offer_Region_Cd = src.OMS_Offer_Region_Cd AND
	...
	SET      
    	OMS_Offer_Region_Nm=src.OMS_Offer_Region_Nm,	
	...
	try {
       ...
       return `Loading of table ${tgt_tbl} Failed with error: ${err}` ;   // Return a error message.
                ...
                // **************        Load for OMS_Offer_Region table ENDs *****************
