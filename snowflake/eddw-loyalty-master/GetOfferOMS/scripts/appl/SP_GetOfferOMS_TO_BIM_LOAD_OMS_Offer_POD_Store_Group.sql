...
--changeset SYSTEM:SP_GetOfferOMS_TO_BIM_LOAD_OMS_Offer_POD_Store_Group runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE SP_GETOFFEROMS_TO_BIM_LOAD_OMS_OFFER_POD_STORE_GROUP(SRC_WRK_TBL VARCHAR(16777216), CNF_DB VARCHAR(16777216), C_PRODUCT VARCHAR(16777216), C_STAGE VARCHAR(16777216))
...
$$
	...
	var tgt_wrk_tbl = `${CNF_DB}.${C_STAGE}.OMS_Offer_POD_Store_Group_WRK`;
	var tgt_tbl = `${CNF_DB}.${cnf_schema}.OMS_Offer_POD_Store_Group`;
	var src_wrk_tmp_tbl = `${CNF_DB}.${C_STAGE}.OMS_Offer_POD_Store_Group_src_wrk`;

                       
    // **************        Load for OMS_Offer_POD_Store_Group table BEGIN *****************
	...
	// Empty the target work table
	    ...
	    var cr_src_wrk_tbl = `	insert into `+ src_wrk_tmp_tbl +`
                            ...
                            ,row_number() over ( PARTITION BY OMS_Offer_Id
                             ...
                             from ` + src_wrk_tbl +` 
                             WHERE OMS_Offer_Id IS NOT NULL AND payload_qualificationStoreGroups_podStoreGroups  IS NOT NULL	
                             )
								...
								FROM (
										SELECT  
										OMS_Offer_Id,
										STOREGROUPS_STORES.value:id::string AS Store_Group_Id,
										filename,
                                        rn
										FROM flat_tmp
										,LATERAL FLATTEN(input => payload_qualificationStoreGroups_podStoreGroups, outer => TRUE ) as STOREGROUPS_STORES
									)   WHERE 
							                			OMS_Offer_Id is not NULL AND
							                			Store_Group_Id is not NULL AND rn =1
   ...
   `;
    try {
        ...
        return "Creation of OMS_Offer_POD_Store_Group src_wrk_tmp_tbl table "+ src_wrk_tmp_tbl +" Failed with error: " + err;   // Return a error message.
        }
...
try {
	...
	try {
       ...
       return `Loading of table ${tgt_tbl} Failed with error: ${err}` ;   // Return a error message.
                ...
                // **************        Load for OMS_Offer_POD_Store_Group table ENDs *****************
