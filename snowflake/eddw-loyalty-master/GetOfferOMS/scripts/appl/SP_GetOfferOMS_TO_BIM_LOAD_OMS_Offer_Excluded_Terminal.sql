...
--changeset SYSTEM:SP_GetOfferOMS_TO_BIM_LOAD_OMS_Offer_Excluded_Terminal runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE SP_GETOFFEROMS_TO_BIM_LOAD_OMS_OFFER_EXCLUDED_TERMINAL(SRC_WRK_TBL VARCHAR(16777216), CNF_DB VARCHAR(16777216), C_PRODUCT VARCHAR(16777216), C_STAGE VARCHAR(16777216))
...
$$
	...
	var tgt_wrk_tbl = `${CNF_DB}.${C_STAGE}.OMS_Offer_Excluded_Terminal_WRK`;
	var tgt_tbl = `${CNF_DB}.${cnf_schema}.OMS_Offer_Excluded_Terminal`;
	var src_wrk_tmp_tbl = `${CNF_DB}.${C_STAGE}.OMS_Offer_Excluded_Terminal_src_wrk`;

                       
    // **************        Load for OMS_Offer_Excluded_Terminal table BEGIN *****************
	    ...
	    var cr_src_wrk_tbl = `	INSERT INTO `+ src_wrk_tmp_tbl +` 
                          with flat_tmp as
                              (		
										...
										FROM ` + src_wrk_tbl +`
										WHERE OMS_Offer_Id IS NOT NULL AND payload_excludedTerminals IS NOT NULL
                                            ...
                                            FROM (
                                                      SELECT
														   ...
														   rn
													 FROM flat_tmp
													 ,LATERAL FLATTEN(input => payload_excludedTerminals, outer => TRUE) as Terminal_groups													   
							                		 ) 				
                                            Where rn = 1 AND
    ...
    try {
        ...
        return "Creation of OMS_Offer_Excluded_Terminal "+ src_wrk_tmp_tbl +" Failed with error: " + err;   // Return a error message.
        }
...
try {
	...
	try {
       ...
       return `Loading of table ${tgt_tbl} Failed with error: ${err}` ;   // Return a error message.
                ...
                // **************        Load for OMS_Offer_Excluded_Terminal table ENDs *****************
