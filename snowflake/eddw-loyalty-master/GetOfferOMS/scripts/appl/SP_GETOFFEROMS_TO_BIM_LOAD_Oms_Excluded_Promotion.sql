...
CREATE OR REPLACE PROCEDURE SP_GETOFFEROMS_TO_BIM_LOAD_Oms_Excluded_Promotion
...
$$
	...
	var tgt_wrk_tbl = `${CNF_DB}.${C_STAGE}.Oms_Excluded_Promotion_WRK`;
	var tgt_tbl = `${CNF_DB}.${cnf_schema}.Oms_Excluded_Promotion`;
	var src_wrk_tmp_tbl = `${CNF_DB}.${C_STAGE}.Oms_Excluded_Promotion_src_wrk`;

                       
    // **************        Load for Oms_Excluded_Promotion table BEGIN *****************
                              ...
                              (		
										...
										WHERE OMS_Offer_Id IS NOT NULL 
                                            ...
                                            FROM (
                                                      SELECT
						                                   OMS_Offer_Id,
							                               F.value::string as Promo_Cd,			 
							                               filename,
														   rn
													 FROM flat_tmp
													 ,LATERAL FLATTEN(input => payload_notCombinableWith, outer => TRUE) as F													   
							                		 ) 				
                                            Where rn = 1 AND
    ...
    try {
        ...
        return "Creation of Oms_Excluded_Promotion "+ src_wrk_tmp_tbl +" Failed with error: " + err;   // Return a error message.
        }
    ...
    snowflake.execute ({ sqlText: sql_truncate_wrk_tbl});
        ...
        ,CASE WHEN DW_First_Effective_dt = CURRENT_DATE  THEN 1 Else 0 END as Sameday_chg_ind
            ...
            WHERE    
             (tgt.OMS_Offer_Id ) in (select distinct PAYLOAD_ID
           FROM ${src_wrk_tbl}
          )
             AND 
...
try {
        ...
        return "Insert of Delete records for OMS_EXCLUDED_PROMOTION work table "+ tgt_wrk_tbl +" Failed with error: " + err;   // Return a error message.
        }
	...
	try {
       ...
       return `Loading of table ${tgt_tbl} Failed with error: ${err}` ;   // Return a error message.
                ...
                // **************        Load for Oms_Excluded_Promotion table ENDs *****************
$$;
