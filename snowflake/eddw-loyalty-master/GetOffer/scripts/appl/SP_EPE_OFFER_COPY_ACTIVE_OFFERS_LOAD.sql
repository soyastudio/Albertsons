...
--changeset SYSTEM:SP_EPE_OFFER_COPY_ACTIVE_OFFERS_LOAD runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME_OUT>>;
use schema <<EDM_DB_NAME_OUT>>.DW_APPL;

CREATE OR REPLACE PROCEDURE <<EDM_DB_NAME_OUT>>.DW_APPL.SP_EPE_OFFER_COPY_ACTIVE_OFFERS_LOAD()
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
    ...
    var src_tbl = src_db + "." + appl_schema + ".EPE_OFFER_JSON_O_STREAM";
   ...
   // var azure_past = "@EDDW_EPE_STAGE_OUTBOUND/epe_offers_delta";
   // var azure_future = "@EDDW_EPE_STAGE_OUTBOUND_PERF/epe_future_offers";
...
/*
    ...
    */
    var filter_copy_future = " WHERE SPLIT_PART(KEY,'|',3) >= DATE(CURRENT_TIMESTAMP)";
    ...
    var sql_copy_future = `copy into ` + azure_future + `
                              from (SELECT to_variant(parse_json(payload)) FROM <<EDM_DB_NAME_OUT>>.DW_DCAT.EPE_OFFER_JSON `+ filter_copy_future+ `)
    ...
    var sql_audit_copy_future = `Insert into ` + audit_table + `
                    (
                      topic, 
                      filename,
                      key, 
                    ...
                    CURRENT_TIMESTAMP() AS dw_create_ts FROM <<EDM_DB_NAME_OUT>>.DW_DCAT.EPE_OFFER_JSON
                    WHERE SPLIT_PART(KEY,'|',3) >= DATE(CURRENT_TIMESTAMP)
                    )`;
	...
	var sub_proc_nms = ['sql_copy_future']
