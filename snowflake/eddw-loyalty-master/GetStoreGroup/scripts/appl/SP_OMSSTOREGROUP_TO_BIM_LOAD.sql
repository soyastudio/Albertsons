...
--changeset SYSTEM:SP_OMSSTOREGROUP_TO_BIM_LOAD runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database EDM_CONFIRMED_PRD;
use schema EDM_CONFIRMED_PRD.DW_APPL;

CREATE OR REPLACE PROCEDURE "SP_OMSSTOREGROUP_TO_BIM_LOAD"()
RETURNS VARCHAR(16777216)
LANGUAGE JAVASCRIPT
EXECUTE AS OWNER
AS 
$$
    var sub_proc_nms = ['sp_OMSStoreGroup_To_BIM_load_OMS_Store_Group', 
						'sp_OMSStoreGroup_To_BIM_load_OMS_Store_Group_Store',
						'SP_OMSStoreGroup_TO_BIM_LOAD_OMSOffer_Flat']
    ...
    try {
        ...
        var ref_schema = metaparams['R_PROMO'];
        var cnf_db = metaparams['CNF_DB'];
        var wrk_schema = metaparams['R_STAGE'];
        var app_schema = metaparams['APPL'];
       
    } catch (err) {
        throw `Error while fetching data from EDM_Environment_Variable_DEV`
    }
    ...
    var src_tbl = `${ref_db}.${app_schema}.OMSStoreGroup_Flat_R_STREAM`;
	var src_rerun_tbl = `${ref_db}.${wrk_schema}.OMSStoreGroup_Flat_Rerun`;
    var src_wrk_tbl = `${ref_db}.${wrk_schema}.OMSStoreGroup_Flat_wrk`;
