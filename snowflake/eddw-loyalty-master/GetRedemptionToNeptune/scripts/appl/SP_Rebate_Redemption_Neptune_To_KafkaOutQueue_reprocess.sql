...
--changeset SYSTEM:SP_Rebate_Redemption_Neptune_To_KafkaOutQueue_reprocess runOnChange:true splitStatements:false OBJECT_TYPE:SP
USE DATABASE <<EDM_DB_NAME>>;
USE SCHEMA DW_APPL;

CREATE OR REPLACE PROCEDURE SP_Rebate_Redemption_Neptune_To_KafkaOutQueue_reprocess(SRC_TBL VARCHAR,CNF_DB VARCHAR,VIEWS_DB VARCHAR,C_STAGE VARCHAR,PRC VARCHAR) 
RETURNS STRING
LANGUAGE JAVASCRIPT
AS 
$$
		...
		var src_nm = 'Rebate_Redemption_Reprocess';
        var reprocess = PRC;
		var tgt_tbl = `${cnf_db}.DW_DCAT.KAFKAOUTQUEUE`;	
		var tgt_wrk_tbl = `${cnf_db}.${wrk_schema}.KAFKA_OUT_QUEUE_REBATE_REDEMPTION_REPROCESS_WRK`;
		...
		var src_retail_store_tbl = `${cnf_db}.${wrk_schema}.retail_store_reprocess_tmp`;
        var src_epe_transaction_header=`${cnf_db}.${wrk_schema}.EPE_TRANSACTION_HEADER_TMP_REBATE_REPROCESS`;
        var src_epe_transaction_item=`${cnf_db}.${wrk_schema}.EPE_TRANSACTION_ITEM_TMP_REBATE_REPROCESS`;
        var src_customer_alternate=`${cnf_db}.${wrk_schema}.CUSTOMER_ALTERNATE_TMP_REBATE_REPROCESS`;		
...
// persist  clips data in work table for the current transaction, includes data from previous failed run
 ...
 try {
      ...
      //  snowflake.execute({ sqlText: sql_crt_src_wrk_tbl });
        ...
        throw `deletion of Source Work table ${src_epe_transaction_header} Failed with error: ${err}`;   // Return a error message.
    }

// persist  clips data in work table for the current transaction, includes data from previous failed run
 var sql_trunc_src_wrk_tbl= `insert into ${src_epe_transaction_header} 
 SELECT T1.* FROM 
(
SELECT 
...
WHERE   STATUS_CD = 'COMPLETED' AND DW_CURRENT_VERSION_IND = true AND DW_LOGICAL_DELETE_IND = false 
  and DW_CREATE_TS>=(CURRENT_DATE - ${reprocess}) and DW_CREATE_TS<(CURRENT_DATE)
 )T1 WHERE RNK=1`;
 try {
        snowflake.execute({ sqlText: sql_trunc_src_wrk_tbl }); 
      //  snowflake.execute({ sqlText: sql_crt_src_wrk_tbl });
...
// persist  clips data in work table for the current transaction, includes data from previous failed run
 ...
 try {
      ...
      //  snowflake.execute({ sqlText: sql_crt_src_wrk_tbl });
        ...
        throw `deletion of Source Work table ${src_customer_alternate} Failed with error: ${err}`;   // Return a error message.
    }

// persist  clips data in work table for the current transaction, includes data from previous failed run
 var sql_trunc_src_wrk_tbl= `insert into ${src_customer_alternate} 
 select 
...
// persist  clips data in work table for the current transaction, includes data from previous failed run
 ...
 try {
      ...
      //  snowflake.execute({ sqlText: sql_crt_src_wrk_tbl });
        ...
        throw `deletion of Source Work table ${src_epe_transaction_item} Failed with error: ${err}`;   // Return a error message.
    }

// persist  clips data in work table for the current transaction, includes data from previous failed run
 var sql_trunc_src_wrk_tbl= `insert into ${src_epe_transaction_item} 
...
WHERE ETI1.DW_CREATE_TS>=(CURRENT_DATE- ${reprocess})
...
AND ETI1.DW_LOGICAL_DELETE_IND = FALSE
 ...
 try {
      ...
      //  snowflake.execute({ sqlText: sql_crt_src_wrk_tbl });
        ...
        throw `Creation of Source Work table ${src_epe_transaction_item} Failed with error: ${err}`;   // Return a error message.
    }	
...
KEY 
FROM 
"${views_db}"."DW_VIEWS"."KAFKAOUTQUEUE" 
