...
--changeset SYSTEM:SP_DIGITAL_REDEMPTION_NEPTUNE_TO_KAFKAOUTQUEUE runOnChange:true splitStatements:false OBJECT_TYPE:SP
USE DATABASE <<EDM_DB_NAME>>;
USE SCHEMA DW_APPL;


CREATE OR REPLACE PROCEDURE "SP_DIGITAL_REDEMPTION_NEPTUNE_TO_KAFKAOUTQUEUE"("SRC_TBL" VARCHAR(16777216), "CNF_DB" VARCHAR(16777216), "VIEWS_DB" VARCHAR(16777216), "C_STAGE" VARCHAR(16777216))
...
$$
        ...
        var kafka_retry_tbl='COUPON_SRV_RESPONSE';
		var tpc_nm='EDDW_C02_ECOMM_CouponService';
		var src_nm='Digital_Redemption';
		var tgt_tbl=`${cnf_db}.DW_DCAT.KAFKAOUTQUEUE`;	
		var tgt_wrk_tbl = `${cnf_db}.${wrk_schema}.KAFKA_OUT_QUEUE_DIGITAL_REDEMPTION_WRK`;
		var src_wrk_tbl = `${cnf_db}.${wrk_schema}.${red_tbl}_src_WRK`;
		var src_rerun_tbl = `${cnf_db}.${wrk_schema}.${red_tbl}_Rerun`;	 	
		var src_clips_tbl = `${cnf_db}.${wrk_schema}.clip_details_tmp_digital`;	
		var src_clp_tbl = `${cnf_db}.${wrk_schema}.RETAIL_CUSTOMER_HH_LOYALTY_PROGRAM`;
...
// persist  clips data in work table for the current transaction, includes data from previous failed run
	var sql_crt_src_wrk_tbl = `DELETE FROM ${src_wrk_tbl}`;
try {
...
// persist stream data in work table for the current transaction, includes data from previous failed run
    ...
    SELECT TXN_ID,CARD_NBR,STORE_ID,TXN_DTE,UPC_ID,MKDN_QTY,MKDN_AMT,REV_DTL_SUBTYPE_ID,INCENTIVE_ID,OMS_OFFER_ID,METADATA$ACTION,METADATA$ISUPDATE,METADATA$ROW_ID
       FROM ${src_tbl}
    UNION ALL
    SELECT TXN_ID,CARD_NBR,STORE_ID,TXN_DTE,UPC_ID,MKDN_QTY,MKDN_AMT,REV_DTL_SUBTYPE_ID,INCENTIVE_ID,OMS_OFFER_ID,METADATA$ACTION,METADATA$ISUPDATE,METADATA$ROW_ID
       FROM ${src_rerun_tbl}`;
    try {
        ...
        return `Creation of Source Work table ${src_wrk_tbl} Failed with error: ${err}`;   
    }
...
// persist  clips data in work table for the current transaction, includes data from previous failed run
	var sql_crt_src_wrk_tbl = `DELETE FROM ${tgt_wrk_tbl}`;
try {
...
SELECT DISTINCT
	...
	FROM (select distinct TXN_ID,TXN_DTE from ${src_wrk_tbl} Where METADATA$ACTION ='INSERT' and REV_DTL_SUBTYPE_ID=6) TFS 
		JOIN (select * from ${views_db}.DW_EDW_VIEWS.TXN_HDR where TXN_DTE >= current_date-1) HDR
		ON TFS.TXN_ID = HDR.TXN_ID
		AND TFS.TXN_DTE = HDR.TXN_DTE
		JOIN (select * from ${views_db}.DW_EDW_VIEWS.TXN_FACTS where MKDN_QTY >0 and TXN_DTE >= current_date-1) TF1
