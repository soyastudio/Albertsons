...
CREATE OR REPLACE PROCEDURE EDM_CONFIRMED_<<ENV>>.DW_APPL.SP_ONETAG_OTHER_TO_BIM_LOAD()
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$

    var sub_proc_nms = ['SP_ONETAG_OTHER_TO_BIM_LOAD_OPERATING_SYSTEM_DETAIL',
                        'SP_ONETAG_OTHER_TO_BIM_LOAD_PAGE',
			'SP_ONETAG_TO_BIM_LOAD_CLICK_STREAM_VISITOR',
		        'SP_ONETAG_TO_BIM_LOAD_CLICK_STREAM_EVENT_MASTER']
    ...
    try {
        ...
        var ref_db = metaparams['REF_DB']; //EDM_REFINED_env
		var ref_schema = metaparams['C_USER_ACT']; //DW_C_USER_ACTIVITY
		var cnf_db = metaparams['CNF_DB']; //EDM_CONFIRMED_env
		var wrk_schema = metaparams['R_STAGE']; //DW_R_STAGE
		var app_schema = metaparams['APPL']; // DW_APPL
		var loc_schema = metaparams['C_CUST']; // DW_C_CUSTOMER
    ...
    function get_args(sub_proc_nm) {
        try {
          ...
          AND PROCEDURE_SCHEMA = '${app_schema}'
    ...
    var src_tbl = `${ref_db}.${app_schema}.ONETAG_OTHER_Flat_R_STREAM`;
	var src_rerun_tbl = `${ref_db}.${wrk_schema}.ONETAG_OTHER_Flat_Rerun`;
    var src_wrk_tbl = `${ref_db}.${wrk_schema}.ONETAG_OTHER_Flat_wrk`;
    ...
    function execSubProc(sub_proc_nm, params) {
        try {
             ret_obj = snowflake.execute ( {sqlText: `call ${cnf_db}.${app_schema}.${sub_proc_nm}(${params})`} ); //while deploying change ${app_schema} to DW_APPL
             ret_obj.next();
