...
CREATE OR REPLACE PROCEDURE SP_GETDOORDASH_TO_BIM_LOAD_PARTNER_GROCERY_ORDER_DETAIL(SRC_WRK_TBL VARCHAR, CNF_DB VARCHAR, C_LOYALTY VARCHAR, C_STAGE VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
...
var tgt_wrk_tbl = `${cnf_db}.${wrk_schema}.PARTNER_GROCERY_ORDER_DETAIL_DOORDASH_WRK`;
...
var sql_command = `INSERT INTO ${tgt_wrk_tbl} 
                   WITH src_wrk_tbl_recs as
							...
							,FileName
							,row_number() over (PARTITION BY order_id,Item_Dsc,UPC_ID ORDER BY Receipt_Nbr asc)as rn
							FROM
							(
							SELECT DISTINCT
							...
							,LPAD(upc_id,13,0) as UPC_Id_Txt
                            ...
                            ,case when length(STORE_TXN_TS) <=16 then try_to_TIMESTAMP(STORE_TXN_TS||':00')
                             else STORE_TXN_TS  end as Store_Transaction_Ts
                      		,LOYALTY_PHONE_NBR as Loyalty_Phone_Nbr
							,lower(user_id) as user_id
							,SNAP_IND as Snap_Ind
							,EBT_SNAP_AMT as Ebt_Snap_Amt
							,EBT_CASH_AMT as Ebt_Cash_Amt
							,UNADJ_ITM_TAX_RATE as Unadjusted_Item_Tax_Rt
							,UNADJ_ITM_TAX_AMT as Unadjusted_Item_Tax_Amt
							,FileName
							FROM  ${src_wrk_tbl}
							WHERE (nvl(UNADJ_ITM_TAX_AMT,'0') <= 99999.9999 AND (nvl(UNADJ_ITM_TAX_AMT,'0') >= -99999.9999))
						   ...
						   UNION ALL
							...
							,FileName
							 FROM ${tgt_exp_tbl}
							 WHERE upper(filename) like '%DOORDASH%' AND 
							 (nvl(UNADJUSTED_ITEM_TAX_AMT,'0') <= 99999.9999 AND (nvl(UNADJUSTED_ITEM_TAX_AMT,'0') >= -99999.9999))
							 )
							 )
							...
							,SRC1.Ebt_Snap_Amt
							,SRC1.Ebt_Cash_Amt
							,SRC1.Unadjusted_Item_Tax_Rt
							,SRC1.Unadjusted_Item_Tax_Amt
					    ...
					    ,src.Ebt_Snap_Amt
						,src.Ebt_Cash_Amt
						,src.Unadjusted_Item_Tax_Rt
						,src.Unadjusted_Item_Tax_Amt
						,src.FileName
						...
						,tgt.Ebt_Snap_Amt
						,tgt.Ebt_Cash_Amt
						,tgt.Unadjusted_Item_Tax_Rt
						,tgt.Unadjusted_Item_Tax_Amt
						...
						OR NVL(to_boolean(src.Snap_Ind),-1) <> NVL(tgt.Snap_Ind,-1)  
						OR NVL(src.Ebt_Snap_Amt,'-1') <> NVL(tgt.Ebt_Snap_Amt,'-1')
						OR NVL(src.Ebt_Cash_Amt,'-1') <> NVL(tgt.Ebt_Cash_Amt,'-1')
						OR NVL(src.Unadjusted_Item_Tax_Rt,'-1') <> NVL(tgt.Unadjusted_Item_Tax_Rt,'-1')
						OR NVL(src.Unadjusted_Item_Tax_Amt,'-1') <> NVL(tgt.Unadjusted_Item_Tax_Amt,'-1')
