...
CREATE OR REPLACE PROCEDURE SP_GETDOORDASH_TO_BIM_LOAD_PARTNER_GROCERY_ORDER_HEADER(SRC_WRK_TBL VARCHAR, CNF_DB VARCHAR, C_LOYALTY VARCHAR, C_STAGE VARCHAR, C_LOCATION VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
	...
	var tgt_wrk_tbl = `${CNF_DB}.${C_STAGE}.PARTNER_GROCERY_ORDER_HEADER_Doordash_WRK`;
...
var sql_command = `INSERT INTO ${tgt_wrk_tbl} 
						   ...
						   ,NET_AMT AS Net_Amt
						   ,'3' AS Partner_Id
						   ,'Doordash' AS Partner_Nm
						   ,STORE_ID AS Store_Id
                           ,ALCOHOL_IND AS Alcoholic_Ind						   
						   ,LOADDATE
...
var truncate_exceptions = `DELETE FROM ${tgt_exp_tbl} WHERE Partner_Id = '3'`; 
						  /*WHERE
