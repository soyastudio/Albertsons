...
--changeset SYSTEM:SP_GETDOORDASH_TO_BIM_LOAD_PARTNER_GROCERY_ORDER_TENDER runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE SP_GETDOORDASH_TO_BIM_LOAD_PARTNER_GROCERY_ORDER_TENDER(SRC_WRK_TBL VARCHAR, CNF_DB VARCHAR, C_LOYALTY VARCHAR, C_STAGE VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
...
var tgt_wrk_tbl = `${CNF_DB}.${C_STAGE}.PARTNER_GROCERY_ORDER_TENDER_DOORDASH_WRK`;
...
,row_number() over (PARTITION BY order_id ORDER BY (select 1)desc )as rn
FROM
(
SELECT DISTINCT
...
,LOADDATE
 ...
 ,MASKED_CREDIT_CARD as Masked_Credit_Card_Nbr
FROM  ${src_wrk_tbl}
UNION ALL
SELECT DISTINCT
      Order_Id
...
FROM `+ tgt_wrk_tbl +`
WHERE upper(filename) like '%DOORDASH%'
AND (Partner_Grocery_Order_Customer_Integration_Id is NULL
or Order_Id is null)
`;
