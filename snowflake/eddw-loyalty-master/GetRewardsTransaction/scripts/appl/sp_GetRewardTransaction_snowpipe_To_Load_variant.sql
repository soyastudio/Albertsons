...
--changeset SYSTEM:sp_GetRewardTransaction_snowpipe_To_Load_variant runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME_R>>;
use schema DW_APPL;

CREATE OR REPLACE PROCEDURE SP_GETREWARDTRANSACTION_SNOWPIPE_TO_LOAD_VARIANT()
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
	...
	var flat_src_tbl = `${ref_db}.${app_schema}.GetRewardTransaction_Flat_R_STREAM`;
	var flat_src_rerun_tbl = `${ref_db}.${wrk_schema}.GetRewardTransaction_Flat_Rerun`;
	...
	var topic_nm_location = 'ESED_C01_RewardTransaction/';
...
/*	//create stage
	...
	var create_variant_tbl = `CREATE TABLE IF NOT EXISTS ${ref_db}.${ref_schema}.${variant_nm}
	(
...
/*	// create pipe  
    ...
    // DDL
	var create_flat_tbl = `CREATE TABLE IF NOT EXISTS ${ref_db}.${ref_schema}.${bod_nm}_Flat (  
        FILENAME   string
        ,BODNm   string
        ,DocumentID	 string
...
,RewardOriginTs	 string
,StatusCd	 string
,TransactionId	 string
,AltTransactionId	 string
,TransactionTypeCd_Code	 string
...
*/
	...
	// Task for FLAT Table
	var create_task = `CREATE OR REPLACE TASK ESED_RewardTransaction_R_TASK
	WAREHOUSE = '${warehouse}'
	SCHEDULE = '1 minutes'
	WHEN
	SYSTEM$STREAM_HAS_DATA('ESED_RewardTransaction_R_STREAM')
	AS 
	call sp_GetRewardTransaction_To_FLAT_load();`
	...
	// To resume the task
	var resume_task = `ALTER TASK ESED_RewardTransaction_R_TASK resume;`
