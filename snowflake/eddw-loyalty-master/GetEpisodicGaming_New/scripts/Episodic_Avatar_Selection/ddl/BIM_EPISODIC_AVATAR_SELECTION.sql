...
--changeset SYSTEM:EPISODIC_AVATAR_SELECTION runOnChange:true splitStatements:false OBJECT_TYPE:TABLE

USE DATABASE <<EDM_DB_NAME>>;
USE SCHEMA <<EDM_DB_NAME>>.DW_C_LOYALTY;

CREATE OR REPLACE TABLE Episodic_Avatar_Selection
(
	Program_Id VARCHAR NOT NULL  COMMENT 'Unique Identifier of the episodic program',
	Avatar_Selection_Id NUMBER NOT NULL  COMMENT 'Unique transaction id for each avatar selection',
	...
	Avatar_Nm VARCHAR NULL  COMMENT 'Name of the avatar selected',
	Times_Selected_Cnt NUMBER NULL  COMMENT 'number of times avatar has been reselected',
	Created_Ts TIMESTAMP NULL  COMMENT 'Record created timestamp on source system',
	Last_Updated_Ts TIMESTAMP NULL  COMMENT 'record last updated timestamp on source system',
	Extract_Ts TIMESTAMP NULL  COMMENT 'record queried timestamp',
...
ALTER TABLE Episodic_Avatar_Selection
	ADD CONSTRAINT XPKEPISODIC_AVATAR_SELECTION PRIMARY KEY (Program_Id, Avatar_Selection_Id, Dw_First_Effective_Dt, Dw_Last_Effective_Dt);
