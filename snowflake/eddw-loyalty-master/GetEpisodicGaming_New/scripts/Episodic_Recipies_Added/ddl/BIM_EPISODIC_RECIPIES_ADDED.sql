...
--changeset SYSTEM:EPISODIC_RECIPIES_ADDED runOnChange:true splitStatements:false OBJECT_TYPE:TABLE

USE DATABASE <<EDM_DB_NAME>>;
USE SCHEMA <<EDM_DB_NAME>>.DW_C_LOYALTY;

CREATE OR REPLACE TABLE EPISODIC_RECIPIES_ADDED
(
	Program_Id VARCHAR NOT NULL  COMMENT 'Unique Identifier of the episodic program',
	Recipe_Added_Id NUMBER NOT NULL  COMMENT 'Unique transaction identifier for each recipie added by user',
	...
	Recipe_Id VARCHAR NULL  COMMENT 'Unique Identifier of the recipie',
	...
	Retail_Store_Id VARCHAR NULL  COMMENT 'Identifier of the user''s preferred store',
	Banner_Nm VARCHAR NULL  COMMENT 'User''s active banner name',
	Division_Nm VARCHAR NULL  COMMENT 'user''s default division name',
	Channel_Nm VARCHAR NULL  COMMENT 'User''s active channel name',
	Access_Type_Dsc VARCHAR NULL  COMMENT 'User''s access type (web or iOS or Android)',
...
ALTER TABLE EPISODIC_RECIPIES_ADDED
	ADD CONSTRAINT XPKepisodic_recipies_added PRIMARY KEY (Program_Id, Recipe_Added_Id, Dw_First_Effective_Dt, Dw_Last_Effective_Dt);
