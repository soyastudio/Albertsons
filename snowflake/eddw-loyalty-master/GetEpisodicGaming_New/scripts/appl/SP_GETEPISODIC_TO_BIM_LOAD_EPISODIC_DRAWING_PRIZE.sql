...
--changeset SYSTEM:SP_GETEPISODIC_TO_BIM_LOAD_EPISODIC_DRAWING_PRIZE runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE <<EDM_DB_NAME>>.DW_APPL.SP_GETEPISODIC_TO_BIM_LOAD_EPISODIC_DRAWING_PRIZE(SRC_WRK_TBL VARCHAR, CNF_DB VARCHAR, C_LOYAL VARCHAR, C_STAGE VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
		...
		var temp_wrk_tbl = `${CNF_DB}.${C_STAGE}.Episodic_Drawing_Prize_tmp_WRK`;
		var cnf_schema = C_LOYAL;
		var tgt_wrk_tbl = `${CNF_DB}.${C_STAGE}.Episodic_Drawing_Prize_WRK`;
		var tgt_tbl = `${CNF_DB}.${cnf_schema}.Episodic_Drawing_Prize`;
		var src_rerun_tbl = `${CNF_DB}.${C_STAGE}.EPISODIC_DRAWING_PRIZES_FLAT_RERUN`;
...
throw "Creation of Source Work table "+ temp_wrk_tbl +" Failed with error: " + err; // Return a error message.
    ...
    // **************        Load for Episodic_Drawing_Prize table BEGIN *****************
								...
								FROM (   SELECT
									...
									,DW_LOGICAL_DELETE_IND
										...
										FROM ( 
											...
											,Row_number() OVER (
											 PARTITION BY Program_Id,Drawing_Id,Prize_Uuid
									...
									and  nvl(src.Prize_Uuid,'-1') = nvl(tgt.Prize_Uuid,'-1')
									WHERE  (
									tgt.Program_Id IS  NULL
									AND tgt.Drawing_Id is  NULL
									AND tgt.Prize_Uuid is NULL
...
try {
		...
		throw `Loading of table ${tgt_tbl} Failed with error: ${err}` ;   // Return a error message.
                ...
                // **************        Load for Episodic_Drawing_Prize Table ENDs *****************
				$$;
