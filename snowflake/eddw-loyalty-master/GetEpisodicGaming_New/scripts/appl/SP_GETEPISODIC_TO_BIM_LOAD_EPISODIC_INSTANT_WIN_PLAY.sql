...
--changeset SYSTEM:SP_GETEPISODIC_TO_BIM_LOAD_EPISODIC_INSTANT_WIN_PLAY runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE <<EDM_DB_NAME>>.DW_APPL.SP_GETEPISODIC_TO_BIM_LOAD_EPISODIC_INSTANT_WIN_PLAY(SRC_WRK_TBL VARCHAR, CNF_DB VARCHAR, C_LOYAL VARCHAR, C_STAGE VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
		...
		var temp_wrk_tbl = `${CNF_DB}.${C_STAGE}.Episodic_Instant_Win_Play_tmp_WRK`;
		var cnf_schema = C_LOYAL;
		var tgt_wrk_tbl = `${CNF_DB}.${C_STAGE}.Episodic_Instant_Win_Play_WRK`;
		var tgt_tbl = `${CNF_DB}.${cnf_schema}.Episodic_Instant_Win_Play`;
		var src_rerun_tbl = `${CNF_DB}.${C_STAGE}.EPISODIC_INSTANTWINPLAYS_FLAT_RERUN`;
...
throw "Creation of Source Work table "+ temp_wrk_tbl +" Failed with error: " + err; // Return a error message.
    ...
    // **************        Load for Episodic_Instant_Win_Play table BEGIN *****************
	...
	var ins_tgt_wrk_table = `INSERT INTO ${tgt_wrk_tbl}
								...
								FROM (   SELECT
										...
										FROM ( 
											...
											,Row_number() OVER (
											 PARTITION BY Program_Id, Play_Id
											 order by to_timestamp_ntz(Extract_Ts) DESC) as rn
											  FROM(
													...
													FROM
													  ...
													  ,Household_Id
													  ,Game_Uuid
													  ,to_timestamp_ntz(PLAY_DATE)	as  Play_Dt
									...
									and  nvl(src.Play_Id,'-1') = nvl(tgt.Play_Id ,'-1')
									WHERE  
									(
										tgt.Program_Id IS  NULL
									AND tgt.Play_Id is  NULL
									...
									(
									 NVL(src.Program_Id,'-1') <> NVL(tgt.Program_Id,'-1')  
									 OR  NVL(src.Play_Id ,'-1') <> NVL(tgt.Play_Id ,'-1')
									 OR  NVL(src.Household_Id,'-1') <> NVL(tgt.Household_Id,'-1')  
									 OR NVL(src.Game_Uuid,'-1') <> NVL(tgt.Game_Uuid,'-1')
									 OR NVL(src.Play_Dt ,'9999-12-31 00:00:00.000') <> NVL(tgt.Play_Dt ,'9999-12-31 00:00:00.000')
...
try {
		...
		throw  `Loading of table ${tgt_tbl} Failed with error: ${err}` ;   // Return a error message.
                ...
                // **************        Load for Episodic_Instant_Win_Play Table ENDs *****************
