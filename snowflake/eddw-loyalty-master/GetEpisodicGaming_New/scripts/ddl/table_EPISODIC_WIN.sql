...
--changeset SYSTEM:EPISODIC_WIN runOnChange:true splitStatements:false OBJECT_TYPE:TABLE
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_C_LOYALTY;

create or replace TABLE EPISODIC_WIN (
	PROGRAM_ID VARCHAR(16777216) NOT NULL COMMENT 'Albertson''s program identifier.',
	WIN_UUID VARCHAR(16777216) NOT NULL COMMENT 'Unique identifier for wins.',
	HOUSEHOLD_ID NUMBER(38,0) COMMENT 'Household ID.',
	GAME_UUID VARCHAR(16777216) COMMENT 'Game won.',
	PRIZE_UUID VARCHAR(16777216) COMMENT 'Prize won.',
	WIN_DT TIMESTAMP_LTZ(9) COMMENT 'The time of win.',
	...
	primary key (PROGRAM_ID, WIN_UUID, DW_FIRST_EFFECTIVE_DT, DW_LAST_EFFECTIVE_DT)
);
