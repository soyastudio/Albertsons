...
--changeset SYSTEM:EPISODIC_OFFER_CLIPPED runOnChange:true splitStatements:false OBJECT_TYPE:VIEW
use database <<EDM_DB_NAME_VIEW>>;
use schema <<EDM_DB_NAME_VIEW>>.DW_VIEWS;

create or replace view EPISODIC_OFFER_CLIPPED(
	Program_Id COMMENT 'Unique Identifier of the episodic program',
	Offer_Clipped_Id COMMENT 'Unique transaction identifier of each offer clipped',
	...
	Oms_Offer_Id COMMENT 'Offer identifier of the clipped offer from oms system',
	...
	Division_Nm COMMENT 'Users default division name',
	Source_Nm COMMENT 'Source name where the action was triggered',
...
) COMMENT='VIEW for EPISODIC_OFFER_CLIPPED' as
select
PROGRAM_ID,
OFFER_CLIPPED_ID,
...
from <<EDM_DB_NAME>>.DW_C_LOYALTY.EPISODIC_OFFER_CLIPPED;
