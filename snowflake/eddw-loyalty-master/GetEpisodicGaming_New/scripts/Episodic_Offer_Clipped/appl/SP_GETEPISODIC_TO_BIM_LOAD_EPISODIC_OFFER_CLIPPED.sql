...
--changeset SYSTEM:SP_GETEPISODIC_TO_BIM_LOAD_EPISODIC_OFFER_CLIPPED runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE SP_GETEPISODIC_TO_BIM_LOAD_EPISODIC_OFFER_CLIPPED(SRC_WRK_TBL VARCHAR, CNF_DB VARCHAR, VIEW_DB VARCHAR, C_LOYAL VARCHAR, C_STAGE VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
	...
	var temp_wrk_tbl = `${CNF_DB}.${C_STAGE}.EPISODIC_OFFER_CLIPPED_TMP_WRK`;
	var cnf_schema = C_LOYAL;
	var tgt_wrk_tbl = `${CNF_DB}.${C_STAGE}.EPISODIC_OFFER_CLIPPED_WRK`;
	var tgt_tbl = `${CNF_DB}.${cnf_schema}.EPISODIC_OFFER_CLIPPED`;
	var src_rerun_tbl = `${CNF_DB}.${C_STAGE}.EPISODIC_OFFER_CLIPPED_FLAT_RERUN`;
	...
	and  tgt.OFFER_CLIPPED_ID is NULL
	) 
	THEN 'I' 
	ELSE 'U' 
	END AS DML_Type
	...
	FROM (
		...
		FROM( 
			...
			Row_number() OVER (
			PARTITION BY PROGRAM_ID,OFFER_CLIPPED_ID order by DW_CREATE_TS DESC,to_timestamp_ntz(EXTRACT_TS) DESC) as rn
			FROM(
				SELECT
				PROGRAM_ID,
				OFFER_CLIPPED_ID,
				...
				FROM
					...
					SELECT  
					PROGRAM_ID, 
					to_number(CLIPPED_OFFER_ID) AS OFFER_CLIPPED_ID, 
					to_number(HOUSEHOLD_ID) AS HOUSEHOLD_ID, 
					OFFER_ID AS OMS_OFFER_ID, 
			...
			) where rn=1 AND PROGRAM_ID is NOT NULL AND OFFER_CLIPPED_ID is NOT NULL
		)  src
	LEFT JOIN
		( 
		SELECT  DISTINCT
		...
		and  nvl(src.OFFER_CLIPPED_ID,'-1') = nvl(tgt.OFFER_CLIPPED_ID ,'-1')
	WHERE(
	tgt.PROGRAM_ID IS  NULL
	AND tgt.OFFER_CLIPPED_ID is  NULL
	)
	...
	OR NVL(src.OMS_OFFER_ID,'-1') <> NVL(tgt.OMS_OFFER_ID,'-1')	
	...
	OR NVL(src.SOURCE_NM,'-1') <> NVL(tgt.SOURCE_NM,'-1')
