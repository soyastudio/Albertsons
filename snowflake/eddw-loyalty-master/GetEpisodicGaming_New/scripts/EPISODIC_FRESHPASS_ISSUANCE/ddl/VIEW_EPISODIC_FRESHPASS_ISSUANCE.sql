...
--changeset SYSTEM:EPISODIC_FRESHPASS_ISSUANCE runOnChange:true splitStatements:false OBJECT_TYPE:VIEW
use database <<EDM_DB_NAME_VIEW>>;
use schema <<EDM_DB_NAME_VIEW>>.DW_VIEWS;

create or replace view EPISODIC_FRESHPASS_ISSUANCE(
	Program_Id COMMENT 'Unique Identifier of the episodic program',
	Freshpass_Issuance_Id COMMENT 'Unique transaction identifier for each freshpass issuance',
	...
	Division_Nm COMMENT 'Users default division',
	...
	Retail_Store_Id COMMENT 'Identifier of user''s preferred store ',
	...
	Extract_Ts COMMENT 'Timestamp when the record has been queried',
...
) COMMENT='VIEW for EPISODIC_FRESHPASS_ISSUANCE' as
select
PROGRAM_ID,
...
from <<EDM_DB_NAME>>.DW_C_LOYALTY.EPISODIC_FRESHPASS_ISSUANCE;
