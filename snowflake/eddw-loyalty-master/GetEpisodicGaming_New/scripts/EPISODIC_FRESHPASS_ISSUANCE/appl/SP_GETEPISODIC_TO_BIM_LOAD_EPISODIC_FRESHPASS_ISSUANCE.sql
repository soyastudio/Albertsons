...
--changeset SYSTEM:SP_GETEPISODIC_TO_BIM_LOAD_EPISODIC_FRESHPASS_ISSUANCE runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE SP_GETEPISODIC_TO_BIM_LOAD_EPISODIC_FRESHPASS_ISSUANCE(SRC_WRK_TBL VARCHAR, CNF_DB VARCHAR, VIEW_DB VARCHAR, C_LOYAL VARCHAR, C_STAGE VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
	...
	var temp_wrk_tbl = `${CNF_DB}.${C_STAGE}.EPISODIC_FRESHPASS_ISSUANCE_TMP_WRK`;
	var cnf_schema = C_LOYAL;
	var tgt_wrk_tbl = `${CNF_DB}.${C_STAGE}.EPISODIC_FRESHPASS_ISSUANCE_WRK`;
	var tgt_tbl = `${CNF_DB}.${cnf_schema}.EPISODIC_FRESHPASS_ISSUANCE`;
	var src_rerun_tbl = `${CNF_DB}.${C_STAGE}.EPISODIC_FRESHPASS_ISSUANCE_FLAT_RERUN`;
	...
	and  tgt.FRESHPASS_ISSUANCE_ID is NULL
	) 
	THEN 'I' 
	ELSE 'U' 
	END AS DML_Type
	...
	FROM (
		...
		FROM( 
			...
			Row_number() OVER (
			PARTITION BY PROGRAM_ID,FRESHPASS_ISSUANCE_ID order by DW_CREATE_TS DESC,to_timestamp_ntz(EXTRACT_TS) DESC) as rn
			FROM(
				SELECT
				PROGRAM_ID,
				FRESHPASS_ISSUANCE_ID,
				...
				FROM
					...
					SELECT  
					PROGRAM_ID, 
					to_number(FP_ISSUANCE_ID) AS FRESHPASS_ISSUANCE_ID, 
			...
			) where rn=1 AND PROGRAM_ID is NOT NULL AND FRESHPASS_ISSUANCE_ID is NOT NULL
		)  src
	LEFT JOIN
		( 
		SELECT  DISTINCT
		...
		and  nvl(src.FRESHPASS_ISSUANCE_ID,-1) = nvl(tgt.FRESHPASS_ISSUANCE_ID ,-1)
	WHERE(
	tgt.PROGRAM_ID IS  NULL
	AND tgt.FRESHPASS_ISSUANCE_ID is  NULL
	)
