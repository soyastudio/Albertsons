...
--changeset SYSTEM:sp_GetOfferRequest_to_BIM_Offer_Request_Offer_Flag runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE <<EDM_DB_NAME>>.DW_APPL.SP_GETOFFERREQUEST_TO_BIM_OFFER_REQUEST_OFFER_FLAG(SRC_WRK_TBL VARCHAR, CNF_DB VARCHAR, CNF_SCHEMA VARCHAR, WRK_SCHEMA VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
...
// ************** Load for Offer_Request_Offer_Flag table BEGIN *****************
// identify if the columns have changed between Target table and changed dataset and create a work table specific to the BIM table.
var tgt_wrk_tbl = cnf_db + "." + wrk_schema + ".Offer_Request_Offer_Flag_wrk";
var tgt_tbl = cnf_db +"."+ cnf_schema +".Offer_Request_Offer_Flag";
var tgt_exp_tbl = cnf_db + "." + wrk_schema + ".Offer_Request_Offer_Flag_Exceptions";
var sql_command = `CREATE OR REPLACE TABLE ` + tgt_wrk_tbl + ` as
WITH src_wrk_tbl_recs as
( SELECT DISTINCT OfferRequestId As Offer_Request_Id
,OfferFlagDsc As Offer_Flag_Dsc
,creationdt
,actiontypecd
,FileName
,Row_number() OVER (partition BY OfferRequestId, OfferFlagDsc ORDER BY To_timestamp_ntz(creationdt) DESC) AS rn
FROM ` + src_wrk_tbl +`
)
...
,src.FileName
,CASE WHEN (tgt.Offer_Request_Id IS NULL AND tgt.Offer_Flag_Dsc IS NULL) THEN 'I' ELSE 'U' END AS DML_Type
...
WHERE   (tgt.Offer_Request_Id IS NULL AND tgt.Offer_Flag_Dsc IS NULL)
OR
...
try {
        ...
        return "Creation of Offer_Request_Offer_Flag work table "+ tgt_wrk_tbl +" Failed with error: " + err;   // Return a error message.
        }
...
snowflake.execute (
        ...
        return "Loading of Offer_Request_Offer_Flag table "+ tgt_tbl + " Failed with error: " + err;   // Return a error message.
        }
...
WHEN Offer_Flag_Dsc is NULL THEN 'Offer_Flag_Dsc is NULL'
ELSE NULL END AS Exception_Reason
,CURRENT_TIMESTAMP AS DW_CREATE_TS
                                            FROM `+ tgt_wrk_tbl +` 
WHERE Offer_Request_Id IS NULL
OR Offer_Flag_Dsc IS NULL`;
    try {
        snowflake.execute (
...
// ************** Load for Offer_Request_Offer_Flag table ENDs *****************
