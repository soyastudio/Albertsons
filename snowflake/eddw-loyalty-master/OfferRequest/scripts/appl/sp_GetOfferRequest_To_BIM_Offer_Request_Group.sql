...
--changeset SYSTEM:sp_GetOfferRequest_To_BIM_Offer_Request_Group runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE <<EDM_DB_NAME>>.DW_APPL.SP_GETOFFERREQUEST_TO_BIM_OFFER_REQUEST_GROUP(SRC_WRK_TBL VARCHAR, CNF_DB VARCHAR, CNF_SCHEMA VARCHAR, WRK_SCHEMA VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$ 
	...
	// **************	Load for Offer_Request_Group table BEGIN *****************
	// identify if the columns have changed between Target table and changed dataset and create a work table specific to the BIM table. 
	var tgt_wrk_tbl = cnf_db + "." + wrk_schema + ".Offer_Request_Group_wrk";
	var tgt_tbl = cnf_db +"."+ cnf_schema +".Offer_Request_Group";
	var tgt_exp_tbl = cnf_db + "." + wrk_schema + ".Offer_Request_Group_Exceptions";
	var sql_command = `CREATE OR REPLACE TABLE ` + tgt_wrk_tbl + ` as 
					...
					(	SELECT DISTINCT OfferRequestId AS Offer_Request_Id,    
										...
										actiontypecd,
										FileName  ,
							Row_number() OVER ( partition BY OfferRequestId, GroupId ORDER BY To_timestamp_ntz(creationdt) DESC) AS rn
						FROM ` + src_wrk_tbl +`
					)
					SELECT 	src.Offer_Request_Id,
							...
							src.FileName 
							,CASE WHEN (tgt.Offer_Request_Id IS NULL AND tgt.Group_Id IS NULL) THEN 'I' ELSE 'U' END AS DML_Type
					...
					LEFT JOIN 	(SELECT tgt.Offer_Request_Id ,
						...
						WHERE  	(tgt.Offer_Request_Id IS NULL AND tgt.Group_Id IS NULL)
						OR 	
						(      	NVL(src.Group_Cd,'-1') <> NVL(tgt.Group_Cd,'-1')
						OR		NVL(src.Group_Nm,'-1') <> NVL(tgt.Group_Nm,'-1')
	...
	try { 
        ...
        return "Creation of Offer_Request_Group work table "+ tgt_wrk_tbl +" Failed with error: " + err;   // Return a error message.
        }
	...
	try {
        ...
        return "Loading of Offer_Request_Group table "+ tgt_tbl + " Failed with error: " + err;   // Return a error message.
        }
	...
	// **************	Load for Offer_Request_Group table table ENDs *****************
