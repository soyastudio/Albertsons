...
--changeset SYSTEM:sp_GetOfferRequest_To_BIM_Offer_Request_POD_Display_Image runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE <<EDM_DB_NAME>>.DW_APPL.SP_GETOFFERREQUEST_TO_BIM_OFFER_REQUEST_POD_DISPLAY_IMAGE(SRC_WRK_TBL VARCHAR, CNF_DB VARCHAR, CNF_SCHEMA VARCHAR, WRK_SCHEMA VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
	...
	// **************	Load for Offer_Request_POD_Display_Image table BEGIN *****************
	// identify if the columns have changed between Target table and changed dataset and create a work table specific to the BIM table. 
	var tgt_wrk_tbl = cnf_db + "." + wrk_schema + ".Offer_Request_POD_Display_Image_wrk";
	var tgt_tbl = cnf_db +"."+ cnf_schema +".Offer_Request_POD_Display_Image";
	var tgt_exp_tbl = cnf_db + "." + wrk_schema + ".Offer_Request_POD_Display_Image_Exceptions";
	var sql_command = `CREATE OR REPLACE TABLE ` + tgt_wrk_tbl + ` as 
						...
						(	SELECT 	DISTINCT OfferRequestId as Offer_Request_Id
											...
											,ImageTypeCd as POD_Image_Type_Cd
											,DisplayImage_ImageId as POD_Image_Id
											, creationdt
											, actiontypecd 
											, FileName
											, Row_number() OVER ( partition BY OfferRequestId, AttachedOfferTypeId, ImageTypeCd ORDER BY To_timestamp_ntz(creationdt) DESC) AS rn
									FROM ` + src_wrk_tbl +`							
						)
						SELECT 	 src.Offer_Request_Id
								,src.User_Interface_Unique_Id
								,src.POD_Image_Type_Cd
								,src.POD_Image_Id								
								
								, src.dw_logical_delete_ind 
								, src.FileName 
								, CASE WHEN (tgt.Offer_Request_Id IS NULL AND tgt.User_Interface_Unique_Id IS NULL AND tgt.POD_Image_Type_Cd IS NULL) THEN 'I' ELSE 'U' END AS DML_Type
								...
								AND tgt.POD_Image_Type_Cd = src.POD_Image_Type_Cd 
								WHERE  (tgt.Offer_Request_Id IS NULL AND tgt.User_Interface_Unique_Id IS NULL AND tgt.POD_Image_Type_Cd IS NULL)
						OR 		(
									  NVL(src.POD_Image_Id,'-1') <> NVL(tgt.POD_Image_Id,'-1')
    ...
    var sql_sameday = `// Processing Sameday updates
						UPDATE ` + tgt_tbl + ` as tgt
						SET		 POD_Image_Id = src.POD_Image_Id
	...
	try {
        ...
        return "Loading of Offer_Request_POD_Display_Image table "+ tgt_tbl + " Failed with error: " + err;   // Return a error message.
        }
			    var truncate_exceptions = `DELETE FROM ${tgt_exp_tbl};`;

		var sql_exceptions = `INSERT INTO ` + tgt_exp_tbl	+`  /*EDM_CONFIRMED_qa.dw_c_STAGE.Offer_Request_POD_Display_Image_Exceptions */
										select  	OFFER_REQUEST_ID ,
													USER_INTERFACE_UNIQUE_ID ,
													POD_IMAGE_TYPE_CD ,
													POD_IMAGE_ID, 
	...
	// **************	Load for Offer_Request_POD_Display_Image table ENDs *****************
