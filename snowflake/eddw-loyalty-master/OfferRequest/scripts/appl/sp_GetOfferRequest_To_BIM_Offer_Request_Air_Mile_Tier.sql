...
--changeset SYSTEM:sp_GetOfferRequest_To_BIM_Offer_Request_Air_Mile_Tier runOnChange:true splitStatements:false OBJECT_TYPE:SP
use database <<EDM_DB_NAME>>;
use schema <<EDM_DB_NAME>>.DW_APPL;

CREATE OR REPLACE PROCEDURE <<EDM_DB_NAME>>.DW_APPL.SP_GETOFFERREQUEST_TO_BIM_OFFER_REQUEST_AIR_MILE_TIER(SRC_WRK_TBL VARCHAR, CNF_DB VARCHAR, CNF_SCHEMA VARCHAR, WRK_SCHEMA VARCHAR)
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
	...
	// **************	Load for Offer_Request_Air_Mile_Tier table BEGIN *****************
	// identify if the columns have changed between Target table and changed dataset and create a work table specific to the BIM table. 
	var tgt_wrk_tbl = cnf_db + "." + wrk_schema + ".Offer_Request_Air_Mile_Tier_wrk";
	var tgt_tbl = cnf_db +"."+ cnf_schema +".Offer_Request_Air_Mile_Tier";
	var tgt_exp_tbl = cnf_db + "." + wrk_schema + ".Offer_Request_Air_Mile_Tier_Exceptions";
	var sql_command = `CREATE OR REPLACE TABLE ` + tgt_wrk_tbl + ` as 
						...
						(	SELECT 	DISTINCT 
											...
											, actiontypecd 
											, FileName
											, Row_number() OVER ( partition BY OfferRequestId, AttachedOfferTypeId, DiscountVersion_DiscountVersionId, AirMileTierNm, ProductGroup_ProductGroupId ORDER BY To_timestamp_ntz(creationdt) DESC) AS rn
									FROM ` + src_wrk_tbl +`
						)
						SELECT 	
								...
								, src.FileName 
								, CASE WHEN (tgt.Offer_Request_Id IS NULL AND tgt.User_Interface_Unique_Id IS NULL AND tgt.Discount_Version_Id IS NULL AND tgt.Air_Mile_Tier_Nm IS NULL AND tgt.Product_Group_Id IS NULL) THEN 'I' ELSE 'U' END AS DML_Type
								...
								AND tgt.Air_Mile_Tier_Nm = src.Air_Mile_Tier_Nm
								AND tgt.Product_Group_Id = src.Product_Group_Id
						WHERE  (tgt.Offer_Request_Id  IS NULL AND tgt.User_Interface_Unique_Id  IS NULL AND tgt.Discount_Version_Id  IS NULL AND tgt.Air_Mile_Tier_Nm  IS NULL AND	tgt.Product_Group_Id IS NULL )
						OR 		(
									 NVL(src.Air_Mile_Point_Qty,'-1') <> NVL(tgt.Air_Mile_Point_Qty,'-1')
	...
	try {
        ...
        return "Creation of Offer_Request_Air_Mile_Tier work table "+ tgt_wrk_tbl +" Failed with error: " + err;   // Return a error message.
        }
						...
						FROM	(	SELECT 	 Offer_Request_Id
									...
									FROM 	`+ tgt_wrk_tbl +` 
										WHERE	DML_Type = 'U'
											...
											AND	Air_Mile_Tier_Nm  IS NOT NULL
    ...
    var sql_sameday = `// Processing Sameday updates
						UPDATE ` + tgt_tbl + ` as tgt
						SET		 Air_Mile_Point_Qty = src.Air_Mile_Point_Qty
	...
	try {
        ...
        return "Loading of Offer_Request_Air_Mile_Tier table "+ tgt_tbl + " Failed with error: " + err;   // Return a error message.
        }
		...
		var sql_exceptions = `INSERT INTO ` + tgt_exp_tbl	+`  
		                               select 	    
												    ...
												    ,CASE 	WHEN Offer_Request_Id is NULL THEN 'Offer_Request_Id is NULL' 
															...
															WHEN Air_Mile_Tier_Nm is NULL THEN 'Air_Mile_Tier_Nm is NULL'
...
{sqlText: sql_rollback  }
	...
	// **************	Load for Offer_Request_Air_Mile_Tier table ENDs *****************
